...
- **GOAL-011**: Implement protocol-core media transfer primitives without framework-level media sugar
  ...
  | TASK-P1-3 | Flood-sleeping requests hold their slot (no backfill — live c6 run showed slot backfill escalates floods 2 s → 15 s), floods never consume the transient retry budget (cap 16/part, uploads too), flood sleeps get a 1 s floor + jitter, and the throttle never reduces on `FloodWait`; disconnects shrink by one and a 50 ms -> 3 ms launch stagger (re-armed on floods) replaces slow start (2026-07-06 master plan). | yes       | 2026-07-07 |
