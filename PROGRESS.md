---
goal: Deliver miniproto 0.1.0 Alpha as a secure async MTProto engine and SDK for Python with bundled Rust acceleration
version: "0.1.0"
...
This file is the root implementation tracker for `miniproto` `0.1.0` Alpha. It consolidates `PLAN.md`, `plans/2026-06-25-implementation-progress.md`, `plans/2026-06-26-package-boundary-and-ecosystem-intent.md`, and `plans/2026-06-29-package-boundary-progress.md` into one actionable plan that future agents must update as work progresses.  
Tracking rules: update the relevant task row when code, docs, tests, and verification for that task are complete; keep the `Completed` column as `yes`, `ready / user action required`, `in progress`, `blocked`, or `no`; record the completion date as `YYYY-MM-DD`; add new tasks only when they are required for Alpha readiness; do not move framework behavior from future `mpgram` into `miniproto`.  
Current baseline on 2026-08-19: the Alpha SDK includes encrypted fail-closed default session storage, atomic domain storage and revisions, generated Layer 228 raw facades/shards from canonical TDLib structure with a validated Telegram Desktop layer, independently pinned core drift/error sources, scheduled/manual upstream freshness reporting, native/fallback protocol paths with parity requirements, stateful native/fallback transport frame pumps for all three TCP modes, opt-in quick ACK with public receipt callbacks and bounded lifecycle correlation, 30 schema-generated Rust TL fast paths, TCP transports and encrypted MTProto runtime, auth/service plumbing, retry/cancellation/disconnect behavior, ordered update recovery and sender-backed pushed-update dispatch, indexed peer resolution, message edit/delete helpers, media upload/download/CDN primitives, shared per-DC/direction byte-weighted scheduling, one ordered streaming download engine, optional plain-download hash verification, fake-server coverage, a normalized deterministic benchmark/acceptance suite, tglib compatibility and resumable live matrix tooling, Linux/Windows manual live automation, expanded Python/Rust/free-threaded CI plus dispatch-only release-artifact automation, twenty installed Python tool CLIs with side-effect-free help, two typed TypeScript documentation CLIs executed through Jiti with side-effect-free help, a progress-reporting canonical release checker, complete maintained Python/Rust documentation, deterministic committed Python/Rust/Telegram references, a searchable Astro Starlight/Pagefind website, provisional Packet Loom branding, history-preserving root `gh-pages` deployment automation, an unprivileged Alpine documentation image, and complete Alpha-facing README/changelog/security/codebase material. Base installs include `cryptography` plus platform-marked `uvloop`/`winloop`; protected-session AES-GCM/Scrypt exposes explicit native and cryptography implementations, selects each native capability for the normal session path, and falls back only when that native capability is unavailable. Waves 0 through 7 of `plans/2026-08-12-v0.1.0-alpha-completion-plan.md` are locally complete. Corrected hosted CI, documentation, and 24-wheel platform runs passed at `c81533922ba63c89337f3959b881109c3b7e5dec`; that evidence closes TASK-082 but predates the current complete release-artifact workflow. TASK-092 remains open for one final-SHA hosted candidate containing the 24 wheels, sdist, crate, attestations, checksums, and manifest. TASK-094 is prepared and awaits user-controlled repository/registry setup and publication.

The intermediary release-readiness bridge between Waves 5 and 6 was locally completed on 2026-08-18. Python, Rust, lockfile, and documentation-site declarations agreed on `0.1.0` Alpha; direct dependencies were audited against their current upstream releases; Ruff, ty, and Griffe were refreshed; Oxfmt/Oxlint covered the supported documentation-site TypeScript sources; maintained JavaScript helpers were converted to typed TypeScript run through Jiti; every Python and documentation helper CLI gained side-effect-free `--help`; the multi-stage release checker printed flushed stage progress; Packet Loom derivatives were regenerated from the canonical minified mark; and the draft-first/history-preserving contribution and deployment contracts were documented. The bridge also fixed cancellation ownership for queued media acquires after any scheduler rebind, rejected container children that were not older than their immediate enclosing MTProto container, and required a Pong to match both the pending request message ID and the original ping ID. At that intermediary checkpoint Wave 6 had not started, so clean distribution metadata review, package artifacts, hosted wheel acceptance, and release handoff were correctly left outside the bridge; the later Wave 6 evidence below supersedes that historical boundary.

Release-automation addendum on 2026-08-19: the dispatch-only wheel workflow is broadened into one complete **Build release artifacts** run that produces 24 wheels, one Python sdist, one build-once Cargo source package, GitHub attestations for every distribution, and an attested deterministic `SHA256SUMS`/`release-manifest.json`. The separate permission-minimized **Publish release** workflow accepts one run ID and exact version, verifies workflow/run/commit/ancestry/version/archive/checksum/attestation provenance, populates a draft GitHub release, publishes through protected PyPI and crates.io OIDC jobs, verifies registry crate bytes, and publishes the GitHub release only last. The `miniproto-release-artifacts` CLI and tests enforce the exact 26-distribution matrix; crates.io metadata and a crate-local README explicitly identify the current `cdylib` as the Python accelerator published for provenance, while direct Rust consumption stays deferred. Actionlint 1.7.12, focused workflow/release/CLI/documentation contracts, Ruff, ty, and an offline local Cargo package extraction/build passed. That Cargo proof used `--allow-dirty` only because the authorized edits are uncommitted and is not release evidence. No manual hosted artifact run, OIDC publisher setting, registry upload, tag, or public release is claimed; Wave 6 clean acceptance and Wave 7 publication remain separate.

Fresh release-automation verification on 2026-08-19: `uv lock --check`, Ruff formatting/lint, and ty passed; the complete Python collection reported 1,113 passed and four opt-in stress skips, with the four credentialed integration tests enabled in the local environment and passing; Cargo formatting/check/strict Clippy and all 27 Rust tests passed; Actionlint 1.7.12 passed every workflow; all release-manifest/workflow/CLI/documentation policy contracts passed; normalized Cargo metadata resolved `miniproto 0.1.0`, `README.md`, crates.io-only publication, and the `miniproto_native` `cdylib`; and the isolated offline dirty-tree package proof extracted and built 12 source files. After regenerating only the expected `_cli.py` source hash in `docs/reference/manifest.json`, the strict documentation gate built 4,584 pages, validated 600,661 local references and seven Pagefind filters, and passed all seven browser tests. No hosted workflow or release-system mutation is included in this local evidence.

Independent-review remediation on 2026-08-20: source-grounded review against the current tree and Telegram's protocol contracts fixed update-cursor gaps, valid DH retry/nonce confirmation, short edit updates, account-DC authorization import, split origin/CDN routing and complete hash-interval verification, bounded one-pass gzip handling, namespace-safe authoritative peer updates, IPv6 proxy authorities, redacted CDN/token material, public Pong correlation, both valid server message-ID classes, piggyback-container aliases, fatal pending-request completion, conservative auth replay, strict local file/media bounds, domain-bound/private session storage, native constant-time/resource/alignment boundaries, and fail-closed resumable PyPI publication. Regression coverage also bounds acknowledgement history, cleans failed transports and closed auxiliary clients, refreshes cached hash metadata, and preserves atomic multi-session destinations. Final local gates passed with 342 formatted Python files, Ruff and ty clean, 1,152 Python tests passed with five opt-in skips (including the four available credentialed integration tests passing), Cargo format/check/strict Clippy clean with 29 Rust tests passed, fresh generated references, a 4,584-page Astro/Pagefind build with 600,661 validated local references and seven filters, and seven browser tests passed. Strict equality against only the current incoming server salt remains intentionally unimplemented because Telegram can legitimately transition salts; adding it safely requires an explicit current/previous-salt history model rather than rejecting authenticated transition traffic.

Wave 6 local acceptance on 2026-08-19: `uv run miniproto-release-check --offline --artifacts-dir .tmp/wave6-20260819-final2` passed all 22 stages without an unexamined failure. The evidence includes fresh schema/reference generation checks; Ruff 0.16.3 formatting/lint; ty 0.0.72; the 1,122-item offline Python collection with 1,114 passed and eight live/opt-in skips; Rust 1.97.1 formatting, strict Clippy, and 27 tests; all nine benchmark/profile families; a fresh CPython 3.14 Windows wheel and sdist; strict archive/Metadata 2.4 inspection; and independent dependency-resolving non-editable installs from both artifacts outside the checkout with native Layer 228 smoke coverage. `twine check` passed both Python distributions. Packaging the Cargo source package from the exact extracted sdist with `cargo package --locked -p miniproto` listed, packaged, extracted, compiled, and verified the intended 11-file crate without `--allow-dirty`. The local artifacts are `.tmp/wave6-20260819-final2/distributions/miniproto-0.1.0-cp314-cp314-win_amd64.whl` (2,152,823 bytes, SHA-256 `b8f3a49485ad06de0a90ff93d9eaafe8c547458367ba0233f8ba9443f1784ea5`), `.tmp/wave6-20260819-final2/distributions/miniproto-0.1.0.tar.gz` (1,864,655 bytes, SHA-256 `baa094b0fea2314837a52ff056c5b96106d859c946a9415b1313625a170177e0`), and `.tmp/wave6-20260819-final2/distributions/miniproto-0.1.0.crate` (37,632 bytes, SHA-256 `c8daf9547d4801dd0d456d487b4f40a92f72eaed8256abd6a577816ac4f48290`). This dirty-tree local run is implementation evidence, not the final attested release candidate: the hosted 24-wheel matrix, credentialed Telegram acceptance, tag, registries, and GitHub release remain unexecuted.

Wave 7 final reconciliation verification on 2026-08-19: after the complete plan/progress/documentation sweep, `uv run miniproto-release-check --offline --artifacts-dir .tmp/wave7-20260819-final` passed all 22 stages with exit code 0. The final working tree again produced 1,114 passing Python tests with eight live/opt-in skips, 27 passing Rust tests, complete maintained Rust documentation for 241 units and 287 explicit parameters, a 4,584-page static documentation artifact with 600,661 validated local references and seven Pagefind filters, all seven browser tests, all nine benchmark/profile families, strict artifact inspection, and independent clean dependency-resolving installs from both distributions. The generated CPython 3.14 Windows wheel is `.tmp/wave7-20260819-final/distributions/miniproto-0.1.0-cp314-cp314-win_amd64.whl` (2,152,823 bytes, SHA-256 `c5e962d9384776ec41af269d4548386da29e4915f5bb4d16fa9593dd09e8820b`); the sdist is `.tmp/wave7-20260819-final/distributions/miniproto-0.1.0.tar.gz` (1,864,655 bytes, SHA-256 `baa094b0fea2314837a52ff056c5b96106d859c946a9415b1313625a170177e0`). The report truthfully records commit `9e7e6093a2cc7b15605b1265616d74b18b055330` with `dirty: true`, so these files are final local implementation evidence only and must not be published. TASK-092 still requires the user-owned clean final SHA to pass the complete hosted 24-wheel/sdist/crate/attestation/sidecar workflow; TASK-094 still requires the documented user-controlled repository, registry, tag, and publication operations.

Fresh intermediary-bridge verification on 2026-08-18: Ruff 0.16.3 formatting/lint and ty 0.0.72 passed; the complete 1,105-test Python collection finished with 1,101 passed and four opt-in stress tests skipped; all nineteen Python entry-point help contracts and both typed Jiti helper help/progress contracts passed; the scheduler and transport-runtime suites passed independently; Cargo formatting/check/strict Clippy plus all 27 Rust tests passed; Oxfmt 0.63.0, Oxlint 1.78.0, pnpm peer checks, Astro diagnostics, and all nine Packet Loom derivative checks passed; committed Python/Rust/Telegram reference freshness passed with Griffe 2.2.0; and the real quick release CLI visibly reported and passed all nine stages, including the complete root-base documentation generation, 4,584-page Astro/Pagefind build, 600,658-reference static validation, and six-test browser acceptance run. No credentialed Telegram run, hosted CI/Pages deployment, clean `0.1.0` distribution build, or manual 24-lane wheel workflow is claimed by this bridge evidence.

Fresh Wave 5 verification on 2026-08-18: schema and committed-reference freshness, Ruff formatting/lint, ty, actionlint 1.7.12, the complete Python collection (1,093 passed and four opt-in stress tests skipped), Cargo formatting/check/strict Clippy, all 27 Rust tests, 62 focused documentation tests, Packet Loom derivative checks, Astro diagnostics, the Node.js 26.7.0/pnpm 11.22.0 production build, 600,658-reference static route/link/search validation, and all six browser tests under both `/` and the exact `/miniproto` Pages base passed. The generated reference contains 517 Python, 62 Rust, and 3,946 Telegram pages; each static build contains 4,584 pages plus Pagefind and sitemap artifacts, while docs CI selects `/miniproto` and publishes the validated bytes at the root of `gh-pages` without rewriting its deployment history. The rebuilt 75,395,136-byte unprivileged NGINX 1.31.3/Alpine 3.24 image became healthy as UID 101, served `/`, `/start/installation/`, and `/pagefind/pagefind.js` with 200 responses, preserved a 404 for an absent route, and returned `Cache-Control: no-cache` for stable Pagefind and social-card assets while reserving long-lived caching for content-hashed `/_astro/` files. The optional source-build target is present but not claimed as locally completed because the Docker daemon exposes only 938 MiB; its documented minimum is 2 GiB. The seven `acquire-codebase-knowledge` deliverables are adapted under `docs/codebase/` with repository frontmatter, site routes, evidence sections, and explicit intent-versus-reality concerns. No credentialed Telegram, hosted CI/Pages deployment, optional in-container source build, or manual 24-lane wheel run is claimed by this local Wave 5 evidence.
...
Wheel/free-threaded follow-up on 2026-08-14: ordinary CI builds no wheels and has a genuine CPython 3.14t source/native job with the GIL forced off. The manual-only wheel workflow has 24 lanes for normal 3.13/3.14 and free-threaded 3.14t across Linux glibc/musl x86_64/aarch64 plus native Windows/macOS x86_64/aarch64. Dependency freshness supersedes the earlier 3.13t request: PyO3 `0.29.2`, `aes-gcm 0.11.0`, `scrypt 0.12.0`, `cryptography 50.0.0`, `uvloop 0.22.1`, and `winloop 0.6.3` are current, while unsupported 3.13t is excluded. The later dependency decision restored `cryptography` and the platform event-loop backend to the default install; both source/free-threaded CI and every manual wheel lane now install and assert those dependencies. Bundled Rust AES-GCM/Scrypt remains native-first per available protected-session capability, with explicit cryptography functions and no fallback retry for real operation failures. The new seventeenth CLI and ninth non-live benchmark family measure native, cryptography, and selected distributions for small/bulk AES-GCM, real Scrypt parameters, and a protected-session round trip. The earlier dependency-free normal/free-threaded wheel hashes and 16-script evidence remain historical but no longer prove the current dependency-resolving acceptance contract. ARMv7 was removed after hosted evidence confirmed that the Rust ARMv7 musl target has no published host tools; aarch64/ARMv8 remains covered for glibc, musl/Alpine, Windows, and macOS. Hosted CI and the manual 24-lane workflow remain unexecuted after that correction.

Dependency/session-crypto verification on 2026-08-14: Ruff format/lint, ty, deterministic schema freshness, actionlint 1.7.12, Cargo formatting, strict all-target/all-feature Clippy, and all 27 Rust tests passed. The canonical offline release check returned exit code 0 in 249.381 seconds: pytest collected 1,021 tests with 1,013 passed and eight intentional live/stress skips; every implemented stage and all nine non-live benchmark commands passed; the Wave 5 docs stage remained correctly pending. A fresh Windows CPython 3.14 wheel installed normally into a clean environment with `cryptography 50.0.0`, `winloop 0.6.3`, `cffi 2.1.1`, and `pycparser 3.0`; its metadata declares `cryptography` plus both platform-marked event-loop requirements, and all 17 installed scripts passed `--help` outside the checkout. The wheel SHA-256 is `90a2bcf0164acd6be6f3ebd3701b91f9b286e60389d4c014e5a1e3703955b4de`; the sdist SHA-256 is `525a804222776f9535d4147b8fbdd6ccc571fc60ffc8ba17634ed0cd513436e6`. A five-run Windows benchmark kept the selected protected-session path on native Rust: the native protected-session round-trip median was 28.7388 ms versus 38.9553 ms for cryptography, and native Scrypt was 28.4210 ms versus 42.6538 ms; cryptography still won the 64 KiB and 1 MiB bulk AES-GCM cases, so no machine-specific size router was introduced. No credentialed Telegram, hosted CI, or manual 24-lane wheel run is claimed.

Hosted wheel failure correction on 2026-08-14: the supplied run showed that `PYTHON_GIL=0` was inherited by normal CPython 3.13/3.14 before interpreter initialization, which those builds reject, and that Linux acceptance searched the container-global `PATH` instead of the selected `/opt/python/<abi>/bin` scripts directory. The workflow now leaves `PYTHON_GIL` unset for every wheel lane, proves 3.14t remains GIL-disabled through runtime assertions after importing miniproto, and resolves every CLI from `sysconfig.get_path("scripts")`. The run log's `--no-deps` and `cryptography is None` checks also prove it executed the earlier workflow revision; the current workflow installs dependencies normally and requires `cryptography` plus the platform event loop. ARMv7 was dropped rather than creating a custom cross-toolchain path for a Rust target without host tools and a weak 32-bit Python wheel ecosystem. Focused workflow contracts passed locally; a new hosted 24-lane dispatch is required for platform acceptance.
...
- **GOAL-013**: Turn local and CI verification into a release-quality gate
  | Task | Description | Completed | Date |
  | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- | ---------- |
  | TASK-082 | Expand automated verification with normal Python 3.13/3.14 plus genuine 3.14t GIL-disabled runtime lanes, schema freshness, docs, benchmark artifacts, sdist, and platform artifacts. Ordinary `.github/workflows/ci.yml` contains no wheel build: it covers both normal Python minors, free-threaded 3.14t with base dependencies, stable Rust 1.97, all nine non-live benchmark artifacts, sdist, and schema freshness. Dispatch-only `.github/workflows/build-wheels.yml`, now named **Build release artifacts**, owns the contract-tested 24-lane wheel matrix for 3.13/3.14/3.14t across Linux glibc/musl x86_64/aarch64 and native Windows/macOS x86_64/aarch64. Hosted workflow-dispatch evidence at `c81533922ba63c89337f3959b881109c3b7e5dec` passed all 18 CI jobs in run `32228452630`, the strict documentation job in run `32228472756`, and all 24 dependency-resolving wheel jobs/artifacts in run `32228463727`. The later build-once sdist/crate/attestation/manifest expansion is release-candidate work tracked by TASK-092 and still requires a final-SHA hosted run. CPython 3.13t and ARMv7 remain deliberately excluded to retain the current dependency/toolchain line. | yes | 2026-08-19 |
  ...
  | TASK-086 | `uv run miniproto-release-check` is the documented non-secret canonical aggregate with quick/offline/live modes, fail-fast or `--keep-going`, fixed ordered stages, streamed exit propagation, environment/artifact hashes, task-owned Windows-safe paths, all nine non-live benchmark families, fresh wheel/sdist and console-script inspection, clean isolated native import, and a strict committed-reference/static-site/search/browser docs stage. | yes | 2026-08-18 |
  | TASK-087 | Run and record the full local verification suite in this file when all release-critical behavior is implemented. The 2026-08-19 canonical offline invocation passed all 22 stages: schema/references, Ruff, ty, 1,114 Python tests with eight live/opt-in skips, Rust formatting/strict Clippy and 27 tests, nine benchmark/profile families, fresh wheel/sdist inspection, and independent clean installs. | yes | 2026-08-19 |
...
- **GOAL-014**: Finish v1 so the package is ready for external testing and release
  | Task | Description | Completed | Date |
  | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------- | ---------- |
  | TASK-088 | Complete the handwritten start/guides/concepts/recipes/FAQ/project information architecture plus committed generated Python/Rust/Telegram reference, seven source-grounded codebase documents, search, theme, Packet Loom assets, root/project-base-safe links and complete sidebar navigation, history-preserving root `gh-pages` automation, an unprivileged Alpine documentation image, portable deployment guidance, and strict docs automation. | yes | 2026-08-18 |
  | TASK-089 | Update `README.md` with Alpha positioning and syntax-checked examples for phone/bot auth, `get_me()`, raw `invoke()`, message/file sends, updates, media, native/session behavior, and the explicit future-`mpgram` boundary. | yes | 2026-08-18 |
  | TASK-090 | Rewrite `CHANGELOG.md` as the complete `v0.1.0 — Alpha` capability/limitation description using the requested monorepo-hash/gitmoji structure without private pre-release chronology or nonexistent compare links. | yes | 2026-08-18 |
  | TASK-091 | Update `SECURITY.md` with the `0.1.x` support policy, private reporting routes, session/auth credential handling, migration/live-test warnings, and dependency/native artifact trust boundaries. | yes | 2026-08-18 |
  | TASK-092 | Build wheels with `uv run maturin build`, verify the package imports from each built wheel, and record artifact paths. The fresh local Windows CPython 3.14 wheel and sdist under `.tmp/wave6-20260819-final2/distributions/` passed strict inspection, Twine, independent dependency-resolving non-editable installs, native/Layer 228 smoke, and the complete 22-stage offline release checker; the post-documentation-sweep run under `.tmp/wave7-20260819-final/` repeated the 22-stage wheel/sdist inspection and clean-install proof on the final working tree. The corrected dispatch-only workflow produces the authoritative attested candidate—24 dependency-resolving wheels, one sdist, one Cargo package, checksums, and a provenance manifest—but that complete hosted run must wait for the user-owned committed/pushed SHA. | in progress | 2026-08-19 |
  | TASK-093 | Verify release metadata for PyPI `miniproto`, crates.io `miniproto`, license, classifiers, project URLs, readme rendering, and source distribution contents. Python wheel/sdist metadata is version `0.1.0`, Alpha, Core Metadata 2.4 with SPDX MIT plus the license file, Python 3.13/3.14 and typed/Rust classifiers, complete project URLs, Markdown readme, runtime and extra dependency boundaries, and the declared native/raw/schema payload. Cargo metadata is also `0.1.0` with the crate-local provenance-only `cdylib` boundary and crates.io fields. Strict wheel/sdist inspection, `twine check`, `cargo package --list`, and package extraction/compilation from the exact sdist all passed. | yes | 2026-08-19 |
  | TASK-094 | Prepare but do not perform the synchronized release. The run-ID-driven publish workflow, OIDC/permission split, build attestation verification, draft-first immutable GitHub release sequence, exact local/remote commands, emergency token paths, post-publication checks, and failure recovery are documented and contract-tested. Required external Trusted Publisher/environment/immutability setup, final-SHA hosted candidate, tag, registry publication, and public release remain user-controlled and unexecuted. | ready / user action required | 2026-08-19 |
...
- **DEP-003**: `ruff==0.16.3`, `ty==0.0.72`, and `pytest==9.1.1` as current Python dev tools
...
- **TEST-008**: Run `uv run maturin build` locally and the dispatch-only complete release-artifact workflow for CPython 3.13/3.14/3.14t wheels across glibc/musl x86_64/aarch64 and native Windows/macOS x86_64/aarch64 plus the authoritative Python sdist, one Cargo source package, checksums, provenance manifest, and attestations
