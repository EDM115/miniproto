...
- **GOAL-014**: Finish v1 so the package is ready for external testing and release
  ...
  | TASK-092 | Build wheels with `uv run maturin build`, verify the package imports from each built wheel, and record artifact paths. The current Windows CPython 3.14 artifact `.tmp/session-crypto-release-20260814-final/distributions/miniproto-0.0.1-cp314-cp314-win_amd64.whl` (SHA-256 `90a2bcf0164acd6be6f3ebd3701b91f9b286e60389d4c014e5a1e3703955b4de`) installed normally with the declared `cryptography 50.0.0` and `winloop 0.6.3` dependencies, imported natively, and exposed 17 scripts whose `--help` paths all passed outside the checkout; the accompanying sdist SHA-256 is `525a804222776f9535d4147b8fbdd6ccc571fc60ffc8ba17634ed0cd513436e6`. Earlier dependency-free/16-script normal and free-threaded artifacts remain historical only. The corrected dispatch-only workflow now produces the complete attested release candidate—24 dependency-resolving wheels, one sdist, one Cargo package, checksums, and a provenance manifest—but final hosted `0.1.0` Wave 6 execution and clean artifact evidence remain outstanding. | in progress | 2026-08-19 |
  | TASK-093 | Verify release metadata for PyPI `miniproto`, crates.io `miniproto`, license, classifiers, project URLs, readme rendering, and source distribution contents. Python/Cargo versions already agree on `0.1.0`; crates.io metadata now includes homepage/docs/readme/keywords/categories and a crates.io-only publication boundary, and the crate-local README documents provenance-only `cdylib` status. A local dirty-tree offline `cargo package` listed, extracted, and built the intended 12-file package, but committed-clean Python wheel/sdist/crate metadata and rendering inspection still belong to Wave 6. | in progress | 2026-08-19 |
