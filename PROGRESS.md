...
- **GOAL-011**: Implement protocol-core media transfer primitives without framework-level media sugar
  ...
  | TASK-P1-3 | Flood-sleeping requests hold their slot (no backfill — live c6 run showed slot backfill escalates floods 2 s → 15 s), floods never consume the transient retry budget (cap 16/part, uploads too), `FLOOD_WAIT_0` gets a 1 s floor while all flood sleeps use only 0–0.3 s jitter, and the throttle never reduces on `FloodWait`; disconnects shrink by one and a flood-aware launch pacer spaces new requests under the recent successful rate after the 50 ms -> 3 ms initial stagger, with stricter `FloodPremiumWait` launch clamping and benchmark launch-pacing counters added after the 2026-07-08 user-mode live run still showed premium-flood stalls (2026-07-06 master plan). | yes       | 2026-07-08 |
