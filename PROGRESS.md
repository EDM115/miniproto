...
- **GOAL-013**: Turn local and CI verification into a release-quality gate
  ...
  | TASK-084 | Add fake-server tests for auth, raw invoke, transport recovery, update gaps, media transfer, and DC migration before relying on live Telegram tests. Wave 1 now covers all production paths available through that wave; later quick-ACK, plain-hash, shared-scheduler, and streaming-cancellation journeys remain required. | in progress | 2026-08-13 |
  | TASK-085 | Add benchmark scripts under `tools/bench/` for native vs fallback crypto, TL serialization, 1k pending RPCs, update dispatch latency, media throughput including opt-in live 2000 MiB Telegram transfer checks, reconnect recovery, and sustained RSS. | in progress | 2026-07-08 |
