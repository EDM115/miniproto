...
Current baseline on 2026-08-13: the pre-alpha SDK includes encrypted fail-closed default session storage, atomic domain storage and revisions, generated Layer 228 raw facades/shards from canonical TDLib structure with a validated Telegram Desktop layer, independently pinned core drift/error sources, scheduled/manual upstream freshness reporting, native/fallback protocol paths with parity requirements, TCP transports and encrypted MTProto runtime, auth/service plumbing, retry/cancellation/disconnect behavior, ordered update recovery and sender-backed pushed-update dispatch, indexed peer resolution, message edit/delete helpers, media upload/download/CDN primitives, shared per-DC/direction byte-weighted scheduling, one ordered streaming download engine, optional plain-download hash verification, fake-server coverage, gated live-test scaffolding, and CI. Waves 0 through 2 of `plans/2026-08-12-v0.1.0-alpha-completion-plan.md` are complete; TASK-084 remains in progress only until the Wave 3 quick-ACK production-path scenario lands, and Wave 3 onward remains incomplete, including native framing/quick ACK/hot TL constructors, the Python 3.13 CI minimum, documentation website, wheel matrix, packaging, and release gates.
...
- **GOAL-013**: Turn local and CI verification into a release-quality gate
  ...
  | TASK-084 | Add fake-server tests for auth, raw invoke, transport recovery, update gaps, media transfer, and DC migration before relying on live Telegram tests. Waves 1 and 2 now cover every production path implemented through Wave 2, including shared-scheduler cap/isolation, iterator early-close cleanup, and plain-hash corruption before output; only the Wave 3 quick-ACK journey remains. | in progress | 2026-08-13 |
  | TASK-085 | Add benchmark scripts under `tools/bench/` for native vs fallback crypto, TL serialization, 1k pending RPCs, update dispatch latency, media throughput including opt-in live 2000 MiB Telegram transfer checks, reconnect recovery, and sustained RSS. Wave 2 adds deterministic shared-scheduler throughput/fairness/cap/cancellation/isolation JSON; the broader automated matrix, loop-lag, tglib compatibility, Windows automation, and remaining acceptance set are still pending. | in progress | 2026-08-13 |
