...
- **GOAL-013**: Turn local and CI verification into a release-quality gate
  | Task     | Description                                                                                                                                                                                                                                            | Completed   | Date       |
  | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------- | ---------- |
  | TASK-082 | Expand `.github/workflows/ci.yml` to run Python 3.13 explicitly, add schema freshness checks, docs build, maturin wheel build, and benchmark smoke artifact upload. Python 3.13/3.14, stable Rust 1.97, normalized benchmark artifacts, Linux/Windows/macOS CPython 3.13/3.14 wheel lanes with clean imports, sdist, and scheduled schema freshness are implemented and contract-tested; the strict docs build/artifact lane remains dependent on Wave 5's site and generators. | in progress | 2026-08-13 |
  ...
  | TASK-085 | Add benchmark scripts under `tools/bench/` for native vs fallback crypto/envelopes/TL, generic TL, 1k pending RPCs, update dispatch latency, deterministic fake media throughput, shared-scheduler fairness/backpressure, reconnect/resend/cancel recovery, sustained RSS/retained objects, and opt-in live 2000 MiB Telegram checks. Reports use `miniproto.benchmark.v1`; smoke/full modes, invariant thresholds, fixed-cadence loop lag, exact tglib payloads, resumable 576-cell full/four-cell smoke matrices, and manual Linux/Windows workflow automation are implemented and tested. Credentialed live results remain a separate gate rather than a completion claim. | yes         | 2026-08-13 |
  | TASK-086 | `uv run python -m tools.release_check` is the documented non-secret canonical aggregate with quick/offline/live modes, fail-fast or `--keep-going`, fixed ordered stages, streamed exit propagation, environment/artifact hashes, task-owned Windows-safe paths, fresh wheel/sdist inspection, and clean isolated native import. Its docs stage reports pending until Wave 5 exists and becomes strict automatically afterward. | yes         | 2026-08-13 |
...
- **GOAL-014**: Finish v1 so the package is ready for external testing and release
  ...
  | TASK-092 | Build wheels with `uv run maturin build`, verify the package imports from the built wheel, and record artifact paths. One fresh Windows CPython 3.14 wheel and sdist passed content/hash inspection and clean isolated native import in Wave 4; the configured multi-OS/3.13/3.14 CI matrix and final `0.1.0` artifacts still need Wave 6 execution. | in progress | 2026-08-13 |
