...
Current baseline on 2026-08-19: the Alpha SDK includes encrypted fail-closed default session storage, atomic domain storage and revisions, generated Layer 228 raw facades/shards from canonical TDLib structure with a validated Telegram Desktop layer, independently pinned core drift/error sources, scheduled/manual upstream freshness reporting, native/fallback protocol paths with parity requirements, stateful native/fallback transport frame pumps for all three TCP modes, opt-in quick ACK with public receipt callbacks and bounded lifecycle correlation, 30 schema-generated Rust TL fast paths, TCP transports and encrypted MTProto runtime, auth/service plumbing, retry/cancellation/disconnect behavior, ordered update recovery and sender-backed pushed-update dispatch, indexed peer resolution, message edit/delete helpers, media upload/download/CDN primitives, shared per-DC/direction byte-weighted scheduling, one ordered streaming download engine, optional plain-download hash verification, fake-server coverage, a normalized deterministic benchmark/acceptance suite, tglib compatibility and resumable live matrix tooling, Linux/Windows manual live automation, expanded Python/Rust/free-threaded CI plus dispatch-only release-artifact automation, twenty installed Python tool CLIs with side-effect-free help, two typed TypeScript documentation CLIs executed through Jiti with side-effect-free help, a progress-reporting canonical release checker, complete maintained Python/Rust documentation, deterministic committed Python/Rust/Telegram references, a searchable Astro Starlight/Pagefind website, provisional Packet Loom branding, history-preserving root `gh-pages` deployment automation, an unprivileged Alpine documentation image, and complete Alpha-facing README/changelog/security/codebase material. Base installs include `cryptography` plus platform-marked `uvloop`/`winloop`; protected-session AES-GCM/Scrypt exposes explicit native and cryptography implementations, selects each native capability for the normal session path, and falls back only when that native capability is unavailable. Waves 0 through 5 implementation of `plans/2026-08-12-v0.1.0-alpha-completion-plan.md` are complete, as are Wave 6's local metadata and canonical offline acceptance. TASK-082 and TASK-092 still need a complete hosted release-artifact workflow run from the eventual committed/pushed SHA; Wave 7 and user-controlled release handoff have not started.

The intermediary release-readiness bridge between Waves 5 and 6 is locally complete on 2026-08-18. Python, Rust, lockfile, and documentation-site declarations now agree on `0.1.0` Alpha; direct dependencies were audited against their current upstream releases; Ruff, ty, and Griffe were refreshed; Oxfmt/Oxlint now cover the supported documentation-site TypeScript sources; maintained JavaScript helpers were converted to typed TypeScript run through Jiti; every Python and documentation helper CLI has side-effect-free `--help`; the multi-stage release checker prints flushed stage progress; Packet Loom derivatives were regenerated from the canonical minified mark; and the draft-first/history-preserving contribution and deployment contracts are documented. The bridge also fixes cancellation ownership for queued media acquires after any scheduler rebind, rejects container children that are not older than their immediate enclosing MTProto container, and requires a Pong to match both the pending request message ID and the original ping ID. Wave 6 has not started: final clean distribution metadata review, package artifacts, hosted wheel acceptance, and release handoff remain outside this bridge.
...
- **GOAL-013**: Turn local and CI verification into a release-quality gate
  | Task | Description | Completed | Date |
  | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- | ---------- |
  | TASK-082 | Expand automated verification with normal Python 3.13/3.14 plus genuine 3.14t GIL-disabled runtime lanes, schema freshness, docs, benchmark artifacts, sdist, and platform artifacts. Ordinary `.github/workflows/ci.yml` contains no wheel build: it covers both normal Python minors, free-threaded 3.14t with base `cryptography`/`uvloop`, stable Rust 1.97, all nine non-live benchmark artifacts, sdist, and schema freshness. Dispatch-only `.github/workflows/build-wheels.yml`, now named **Build release artifacts**, owns the contract-tested 24-lane wheel matrix for 3.13/3.14/3.14t across Linux glibc/musl x86_64/aarch64 and native Windows/macOS x86_64/aarch64, with clean dependency-resolving installation, `cryptography`/platform-loop assertions, ABI/GIL assertions, concurrent native calls, protected string sessions, and all installed-script help. It also creates one authoritative Python sdist, one build-once Cargo source package, per-distribution GitHub attestations, and attested checksum/provenance sidecars. It leaves `PYTHON_GIL` unset so normal interpreters can initialize, verifies 3.14t's actual runtime state after native import, and resolves scripts from the selected interpreter rather than the runner/container-global `PATH`. CPython 3.13t and ARMv7 are deliberately excluded to retain the current dependency/toolchain line. The 2026-08-14 correction creates pytest's basetemp parent and applies the affected uvloop/winloop debug-runner workaround to both supported normal Python minors. The Wave 5 strict `.github/workflows/docs.yml` build/search/browser/artifact/Pages lane is implemented and locally validated on its pinned toolchain. A fresh hosted CI rerun and manual complete release-artifact workflow run remain outstanding. | in progress | 2026-08-19 |
...
- **GOAL-014**: Finish v1 so the package is ready for external testing and release
  ...
  | TASK-094 | Prepare but do not perform the synchronized release. The run-ID-driven publish workflow, OIDC/permission split, build attestation verification, draft-first immutable GitHub release sequence, exact local/remote commands, emergency token paths, and failure recovery are documented and contract-tested. Required external Trusted Publisher/environment/immutability setup, final hosted evidence, tag, registry publication, and public release remain user-controlled and unexecuted. | in progress | 2026-08-19 |
