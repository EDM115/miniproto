...
The native wheel has no hard Python dependency and is tested on normal CPython 3.13/3.14 plus free-threaded CPython 3.14t with the GIL actually disabled. CPython 3.13t is not supported because the project prioritizes the current PyO3 dependency line. Optional `miniproto[event-loop]` installs the supported `uvloop`/`winloop` acceleration path, while `miniproto[crypto-fallback]` installs the CFFI-backed `cryptography` fallback. Manual wheel automation covers Linux glibc and musl on x86_64, aarch64/ARMv8, and armv7l/ARMv7, plus native Windows/macOS x86_64 and aarch64.
