...
- The Wave 5 brand gate now has four self-contained SVG families under `docs-site/src/assets/brand/concepts`: Protocol Aperture, Duplex Rails, Packet Loom, and Clocked Core. Do not select one implicitly or build final theme/homepage/README brand assets from the raster ideation boards; wait for the maintainer's explicit family choice, then refine the selected `concept.svg` into the production source.
