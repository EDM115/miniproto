...
language: "telegram"
kind: "function"
qualified_name: "channels.exportMessageLink"
...
| grouped | flags.0?true | flags.0 | — | No description provided by the pinned schema. |
| thread | flags.1?true | flags.1 | — | No description provided by the pinned schema. |
...
| --- | ---: | --- |
| grouped | 0 | Controlled by `flags`; present when this bit is set. |
| thread | 1 | Controlled by `flags`; present when this bit is set. |

## Python binding

```python
from miniproto.raw.functions import ChannelsExportMessageLink
```

Public access: `miniproto.raw.functions.ChannelsExportMessageLink`.
...
request_type = ChannelsExportMessageLink
```
