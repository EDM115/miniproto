...
language: "telegram"
kind: "function"
qualified_name: "channels.toggleSignatures"
...
| signatures_enabled | flags.0?true | flags.0 | — | No description provided by the pinned schema. |
| profiles_enabled | flags.1?true | flags.1 | — | No description provided by the pinned schema. |
...
| --- | ---: | --- |
| signatures_enabled | 0 | Controlled by `flags`; present when this bit is set. |
| profiles_enabled | 1 | Controlled by `flags`; present when this bit is set. |

## Python binding

```python
from miniproto.raw.functions import ChannelsToggleSignatures
```

Public access: `miniproto.raw.functions.ChannelsToggleSignatures`.
...
request_type = ChannelsToggleSignatures
```
