...
language: "telegram"
kind: "function"
qualified_name: "messages.createChat"
...
| ttl_period | flags.0?int | flags.0 | — | No description provided by the pinned schema. |

## Flags

| Parameter | Bit | Meaning |
| --- | ---: | --- |
| ttl_period | 0 | Controlled by `flags`; present when this bit is set. |

## Python binding

```python
from miniproto.raw.functions import MessagesCreateChat
```

Public access: `miniproto.raw.functions.MessagesCreateChat`.
...
request_type = MessagesCreateChat
```
...
| 400 | [`USERS_TOO_FEW`](/reference/telegram/errors/users-too-few/) | Not enough users (to create a chat, for example). |
...
| 500 | [`CHAT_ID_GENERATE_FAILED`](/reference/telegram/errors/chat-id-generate-failed/) | Failure while generating the chat ID. |
