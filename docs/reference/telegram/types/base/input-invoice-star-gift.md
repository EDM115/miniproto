...
language: "telegram"
kind: "type"
qualified_name: "inputInvoiceStarGift"
...
| include_upgrade | flags.2?true | flags.2 | — | No description provided by the pinned schema. |
...
| include_upgrade | 2 | Controlled by `flags`; present when this bit is set. |
| message | 1 | Controlled by `flags`; present when this bit is set. |

## Python binding

```python
from miniproto.raw.types import InputInvoiceStarGift
```

Public access: `miniproto.raw.types.InputInvoiceStarGift`.
...
constructor_type = InputInvoiceStarGift
```
