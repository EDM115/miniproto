...
language: "telegram"
kind: "type"
qualified_name: "inputMediaDocumentExternal"
...
| video_cover | flags.2?InputPhoto | flags.2 | — | No description provided by the pinned schema. |
| video_timestamp | flags.3?int | flags.3 | — | No description provided by the pinned schema. |
...
| video_cover | 2 | Controlled by `flags`; present when this bit is set. |
| video_timestamp | 3 | Controlled by `flags`; present when this bit is set. |

## Python binding

```python
from miniproto.raw.types import InputMediaDocumentExternal
```

Public access: `miniproto.raw.types.InputMediaDocumentExternal`.
...
constructor_type = InputMediaDocumentExternal
```
