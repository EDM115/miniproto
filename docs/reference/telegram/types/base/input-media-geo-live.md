...
language: "telegram"
kind: "type"
qualified_name: "inputMediaGeoLive"
...
| stopped | flags.0?true | flags.0 | — | No description provided by the pinned schema. |
| geo_point | InputGeoPoint | — | — | No description provided by the pinned schema. |
| heading | flags.2?int | flags.2 | — | No description provided by the pinned schema. |
...
| --- | ---: | --- |
| stopped | 0 | Controlled by `flags`; present when this bit is set. |
| heading | 2 | Controlled by `flags`; present when this bit is set. |
...
```python
from miniproto.raw.types import InputMediaGeoLive
```

Public access: `miniproto.raw.types.InputMediaGeoLive`.
...
constructor_type = InputMediaGeoLive
```
