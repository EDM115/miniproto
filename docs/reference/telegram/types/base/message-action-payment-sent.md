...
language: "telegram"
kind: "type"
qualified_name: "messageActionPaymentSent"
...
| invoice_slug | flags.0?string | flags.0 | — | No description provided by the pinned schema. |
...
| invoice_slug | 0 | Controlled by `flags`; present when this bit is set. |
| subscription_until_date | 4 | Controlled by `flags`; present when this bit is set. |

## Python binding

```python
from miniproto.raw.types import MessageActionPaymentSent
```

Public access: `miniproto.raw.types.MessageActionPaymentSent`.
...
constructor_type = MessageActionPaymentSent
```
