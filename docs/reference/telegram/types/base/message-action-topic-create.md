...
language: "telegram"
kind: "type"
qualified_name: "messageActionTopicCreate"
...
| title_missing | flags.1?true | flags.1 | — | No description provided by the pinned schema. |
...
| --- | ---: | --- |
| title_missing | 1 | Controlled by `flags`; present when this bit is set. |
| icon_emoji_id | 0 | Controlled by `flags`; present when this bit is set. |

## Python binding

```python
from miniproto.raw.types import MessageActionTopicCreate
```

Public access: `miniproto.raw.types.MessageActionTopicCreate`.
...
constructor_type = MessageActionTopicCreate
```
