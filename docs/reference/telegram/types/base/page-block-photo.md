...
| webpage_id | flags.0?long | flags.0 | — | No description provided by the pinned schema. |
...
| webpage_id | 0 | Controlled by `flags`; present when this bit is set. |

## Python binding

```python
from miniproto.raw.types import PageBlockPhoto
```

Public access: `miniproto.raw.types.PageBlockPhoto`.
...
constructor_type = PageBlockPhoto
```
