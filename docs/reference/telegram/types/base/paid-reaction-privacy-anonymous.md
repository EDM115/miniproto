---
title: "paidReactionPrivacyAnonymous"
description: "No description provided by the pinned schema."
generated: true
editUrl: false
language: "telegram"
kind: "type"
qualified_name: "paidReactionPrivacyAnonymous"
source_path: "tools/schema/schema.json"
source_url: "https://github.com/EDM115/miniproto/blob/master/tools/schema/schema.json"
namespace: "base"
schema_source: "tdlib"
constructor_id: "0x1f0c1ad9"
---

# `paidReactionPrivacyAnonymous`

No description provided by the pinned schema.

## Signature

```tl
paidReactionPrivacyAnonymous#1f0c1ad9 = PaidReactionPrivacy;
```

## Result type

`PaidReactionPrivacy`

## Parameters

| Name | Type | Flag | Default | Description |
| --- | --- | --- | --- | --- |
| — | — | — | — | This declaration has no parameters. |

## Python binding

```python
from miniproto.raw.types import PaidReactionPrivacyAnonymous
```

Public access: `miniproto.raw.types.PaidReactionPrivacyAnonymous`.

## Safe usage shape

```python
from miniproto.raw.types import PaidReactionPrivacyAnonymous

# Naming the raw constructor class is local only; it performs no I/O or network request.
constructor_type = PaidReactionPrivacyAnonymous
```

## Result family

[`PaidReactionPrivacy`](/reference/telegram/types/results/paid-reaction-privacy/)

## Relationships

- Result family: [`PaidReactionPrivacy`](/reference/telegram/types/results/paid-reaction-privacy/)
- Related constructors: [`paidReactionPrivacyDefault`](/reference/telegram/types/base/paid-reaction-privacy-default/), [`paidReactionPrivacyPeer`](/reference/telegram/types/base/paid-reaction-privacy-peer/)
- Accepted by: [`messages.sendPaidReaction`](/reference/telegram/functions/messages/send-paid-reaction/), [`messages.togglePaidReactionPrivacy`](/reference/telegram/functions/messages/toggle-paid-reaction-privacy/), [`updatePaidReactionPrivacy`](/reference/telegram/types/base/update-paid-reaction-privacy/)

## Provenance

- structural source: `tdlib`
- canonical schema: https://raw.githubusercontent.com/tdlib/td/refs/heads/master/td/generate/scheme/telegram_api.tl
- prose merge precedence: TDLib → Telegram Desktop → Core JSON
- source note: Canonical structure is the pinned TDLib telegram_api.tl; Layer 229 comes only from the matching Telegram Desktop end-of-file marker; core.telegram.org inputs enrich documentation and RPC error metadata without overriding structure.

## Source-diff notes

- tdlib_vs_core: changed_count=78; core_only_count=18; overlap_count=2285; tdlib_only_count=195
- tdlib_vs_tdesktop: changed_count=0; overlap_count=2470; tdesktop_only=null; tdlib_only=accessPointRule, help.configSimple, inputPeerPhotoFileLocationLegacy, inputStickerSetThumbLegacy, invokeWithApnsSecretPrefix, invokeWithBusinessConnectionPrefix, invokeWithGooglePlayIntegrityPrefix, invokeWithReCaptchaPrefix, ipPort, ipPortSecret
