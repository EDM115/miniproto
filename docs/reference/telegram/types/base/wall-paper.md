...
| pattern | flags.3?true | flags.3 | — | No description provided by the pinned schema. |
...
| pattern | 3 | Controlled by `flags`; present when this bit is set. |
...
```python
from miniproto.raw.types import WallPaper
```

Public access: `miniproto.raw.types.WallPaper`.
...
- Related constructors: [`wallPaperNoFile`](/reference/telegram/types/base/wall-paper-no-file/)
