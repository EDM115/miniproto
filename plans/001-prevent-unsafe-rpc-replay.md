...
- Base commit: cf7db29
- Dependencies: none
- Dependents: plans 002 and 008 rely on logical request aliases and predictable sender request ownership
...
RawSender, every repository sender implementation, and every typed test double must accept the explicit keyword. This is an intentional pre-alpha interface change; do not hide an incomplete migration behind `**kwargs`.
...
- A failure before _send_pending registers or calls transport.send follows the existing immediate send-error path.
- Once transport.send is invoked, any transport failure is conservatively treated as potentially written. An unsafe request becomes ambiguous even when the failure is raised by write/drain itself; it must not enter the reconnect resend path.
...
- Server-correlated `bad_msg_notification` and `bad_server_salt` retries remain a separate protocol path because the server explicitly rejected the referenced message. Retry only when `bad_msg_id` identifies a currently known recent pending alias, and preserve the logical request’s retry metadata.
- Keep `msg_resend_req` handling separate from ordinary reconnect recovery. The official service-message contract explicitly requires immediate retransmission of a requested message, so a currently known pending alias remains eligible regardless of ordinary `retry_safe`; route it through the same cancellation-safe logical alias replacement helper. Unknown or forgotten `msg_resend_req` handling is unchanged by this plan.
...
- Cancellation during alias replacement removes every alias or restores one valid alias before the cancellation escapes, so no unresolved future becomes unreachable.
...
| src/miniproto/**init**.py          | Export the public exception if public errors are re-exported there |
...
- Inventory and deliberately update every in-repository RawSender implementation and test double. Document the pre-alpha custom-sender signature migration for downstream users.
- If a transport cannot report whether bytes reached the socket, use the conservative boundary at the call to Transport.send; do not replay an unsafe request after any error from that call.
...
5. Split _resend_pending into safe resend and unsafe failure paths. Apply the same safety decision to transport errors raised after Transport.send begins. Remove all stale aliases before completing an unsafe future; make alias replacement cancellation-safe and keep existing attempt limits for safe requests.  
...
- Transport write/drain failure after send begins is ambiguous for unsafe requests and never replays them.
- Correlated bad-message rejection can retry without weakening ordinary reconnect safety; unknown or stale bad_msg_id cannot mutate or replay pending state.
...
- Implemented logical pending request identity with multiple message-id aliases, terminal alias cleanup, and explicit `retry_safe` propagation through sender, invoke, client, internal calls, and typed test doubles.
- Unsafe RPCs now raise redaction-safe `AmbiguousRpcResult` after ambiguous transport entry and are not replayed; safe RPCs reconnect with a freshly encoded message id while late responses to old aliases remain resolvable.
- Known `bad_msg_notification` and `bad_server_salt` messages alone can mutate/retry state; known `msg_resend_req` aliases are retransmitted through the logical request helper regardless of ordinary retry safety.
- Focused verification passed: 47 sender-runtime tests, 32 invoke tests, dedicated ambiguity/service/cancellation/redaction tests, scoped Ruff, scoped type checking, and Python compilation.
- Independent rereview passed the four review regressions, the current 83-test sender/invoke scope, scoped Ruff format/lint, type checking, and Python compilation with no required findings.
