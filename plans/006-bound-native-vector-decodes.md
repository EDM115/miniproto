...
- TDD RED: malformed-count tests failed because both compiled native decoders allocated/iterated until scalar truncation and returned the old `TL data ended before the requested value could be decoded` error.
- Python fallback now validates count against remaining payload before its first list append/loop; Rust validates checked header/count/payload arithmetic before `try_reserve_exact`.
- Rust encode paths reject counts beyond i32 and use checked, fallible byte-capacity reservation.
- Rebuilt the editable native extension in release mode with `maturin develop --release --uv`; the focused suite then exercised compiled bytes paths and fallback memoryview paths.
- Independent review found and fixed the remaining offset-parity gaps: negative, near-end (`len(data) - 7` and `len(data) - 1`), and oversized offsets now produce the same ValueError and stable truncation phrase through compiled bytes and fallback memoryview paths. Rust also rejects element-capacity overflow as ValueError before allocator reservation.
- Focused Python verification after the review fixes: 109 passed across test_tl_codec.py, test_native.py, test_native_parity.py, and test_performance_primitives.py.
- Rust verification exited zero: cargo fmt --check, cargo clippy --all-targets --all-features -- -D warnings, and cargo test --all-features with 15 tests.
- Ruff format/lint and ty checks exited zero.
- Release-mode runtime smoke exited zero; relevant medians were 106.040 ms for 10,000 upload.getFile encodes and 10.071 ms for 100 upload.File decodes, with no observed hot-path regression against the previous local smoke.
