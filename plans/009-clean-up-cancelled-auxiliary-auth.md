...
- the auxiliary is connected and fully usable;
- client is inserted into _auxiliary_download_clients.
  ...
  An auxiliary recovered from an existing sibling session follows the same rule: do not insert it into the cache before `connect()` succeeds. If connection is cancelled or fails, remove any provisional entry and disconnect/close the locally owned client.
...
- On success, connect, register, and clear local ownership.
...
- Every cached auxiliary has completed a successful connect at least once; a failed first connect cannot leave a cache entry.
...
3. Refactor new-auxiliary construction, authorization, first connect, and registration into one ownership-guarded block. Register only after successful connect, and remove duplicated direct storage.close/disconnect branches only after equivalent reason events are preserved.  
...
- Cancellation/failure during the first connect of both newly authorized and recovered sibling sessions.
...
- Added locally owned auxiliary setup through validated registration, one shielded and joined disconnect helper, and graceful redacted handling for ordinary initial load/connect/registration failures.
- Deterministic cancellation tests cover authorization, post-auth load, recovered and newly authorized first connect, registration, repeated cancellation during cleanup, cleanup errors, identity/token failures, task/storage/sender cleanup, and protection of previously cached healthy clients.
- Independent review found and the repair corrected double disconnect when cancellation arrived during branch cleanup, plus ordinary setup failures that incorrectly escaped instead of degrading to fewer sessions.
- Final focused evidence includes 97 media tests, 15 auxiliary lifecycle tests, scoped Ruff format/lint, scoped type checking, schema freshness, and independent approval.
