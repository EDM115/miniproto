...
Plan 004’s storage contract exposes a cheap synchronous `domain_revisions()` snapshot. Before lookup, PeerCache compares the `auth` revision, which contains the current user identity, and the `peers` revision. Rebuild once under lock only when either changed; update_state-only and metadata-only mutations must not invalidate peer indexes.
...
- Implemented revision-aware in-memory peer indexes, committed merge metadata, canonical incremental reconciliation, TTL-aware aliases, and bounded rebuild fallback across `peers.py`, `peer_merge.py`, storage integration, tests, and the runtime benchmark.
- Post-fix focused proof: 99 tests passed; the named Plan 004 subset passed 6 tests; cached-wrapper/backend loads were 1/1, canonical visits 10,000, rebuilds 1, and direct-update rebuild delta 0. Independent re-review: APPROVE / READY.
- Final benchmark results: kind 103.65x, numeric 106.79x, username 2427.63x, phone 642.74x, combined 1210.31x; incremental heap ratio 1.763; incremental reconciliation 92.300 us.
