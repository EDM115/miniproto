...
The installed Windows backend is `winloop 0.6.3` and exposes `new_event_loop()`, so the factory-first design is supported for normal execution. A deterministic local reproduction on Python 3.14.5 showed that `asyncio.Runner(debug=True, loop_factory=winloop.new_event_loop)` crashes natively when finalizing an unfinished async generator. The same failure was reproduced in CI and on a Linux VPS with CPython 3.14.6 and `uvloop 0.22.1`; it matches upstream [MagicStack/uvloop#715](https://github.com/MagicStack/uvloop/issues/715). Debug-only execution therefore uses the stdlib Runner through these affected backend versions, while non-debug execution keeps the optimized factory.
...
- Post-completion CI correction on 2026-07-17: CPython 3.14.6 on Linux reproduced the upstream uvloop debug-mode async-generator finalization segfault (exit 139) with `uvloop 0.22.1`, while the same reproducer passed with stdlib debug mode and with uvloop non-debug mode. A focused regression test proved the old guard covered winloop only, then passed after extending the version-bounded stdlib debug fallback to uvloop; normal execution continues to use the optimized factory.
