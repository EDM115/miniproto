...
- Drift check: reopen pyproject.toml, src/miniproto/event_loop.py, src/miniproto/__init__.py, tests/test_event_loop.py, docs/development.md, and current Python 3.16 asyncio documentation. Stop if import no longer mutates policy and run already uses a supported loop_factory-only path.
## 1. Objective
...
| src/miniproto/__init__.py | Preserve event_loop export without triggering backend work |
