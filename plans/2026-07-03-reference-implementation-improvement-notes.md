...
- Concurrent downloads now have adaptive throttling: generic flood waits sleep in place without cutting the active request window, `FloodPremiumWait` switches the current adaptive transfer to one active request, transient disconnect/timeouts trim by one, and successful chunks slowly ramp back up after non-premium reductions. This is a small safe step toward mtcute-style media throttling without introducing media-session pools yet.
