...
- Done 2026-07-07 (REVISED same day after a live failure): the first implementation released a flood-sleeping task's slot+bytes back to the window and backfilled it with new requests. The first live c6/l2 run falsified that design: six requests flooded together, their freed slots were instantly backfilled, the request rate never dropped, and the server escalated FLOOD_WAIT_2 → FLOOD_WAIT_15 (8161 part retries by the 37 MiB mark) until one part burned its whole retry budget on consecutive floods and aborted the transfer. Final behavior: **flood-sleeping tasks HOLD their slot** (mtcute's fixed-slot model — pressure drops naturally, other slots keep flowing); **floods never consume the transient retry budget** (separate cap of 16 per part, uploads too — the download abort path is gone); **flood sleeps get a 1 s floor plus 0–1 s jitter** so lockstep-flooded requests de-synchronize (FLOOD_WAIT_0/1 retried instantly just re-triggered the flood); the stagger re-arms to 50 ms after any flood; and delegated-flood `rpc.invoke` logs demoted to INFO (the 3-lines-per-flood ERROR wall is gone). Throttle behavior per user decision stands: never reduce on `FloodWait` (growth pause only), disconnect/timeout errors shrink by one with a 2 s cooldown, no slow start.
