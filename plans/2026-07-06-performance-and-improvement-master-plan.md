...
> **P1 follow-up (2026-07-08, user/DC4/VPS/uvloop, 1000 MiB):** `--size 1000mib` now produces a real 1000 MiB payload. Upload stayed healthy at 17.1 MiB/s transfer speed, but default download still collapsed to 2.8 MiB/s with 48 flood waits, 28 of them `FloodPremiumWait` (96 s premium wait), while the c1/l1 fixed-1MiB comparison remained the safer shape. The v7 fix therefore keeps generic flood pacing bounded and switches the current adaptive download to one active request after `FloodPremiumWait`. The combined upload+download trace also showed RSS +300 MB while tracemalloc current was low, so run an upload-only `--trace-memory` sample before changing memory ownership code; a high combined RSS delta may be allocator retention or sequential upload+download high-water behavior rather than a live Python leak.
...
> **Status (2026-07-07): all ten P1 tasks are implemented and covered by unit/fake-server tests** (284 passed, 4 skipped; ruff/ty/schema clean). Per-task `Done:` notes below. Live validation on the VPS is the remaining step — the bench defaults now match TASK-P1-1 (c6/l2/512K→1M adaptive/8 MiB window), so a plain `--operation both` run exercises the new pipeline.
...
- Done 2026-07-07: library defaults are now `concurrency=6`, `media_lanes=2`, `part_size=512 KiB` growing adaptively to `max_part_size=1 MiB`, and an 8 MiB rolling byte-window floor (`max(8 MiB, part_size*concurrency)` when `max_in_flight_bytes` is unset). `download_media()` resolves `total_size` from Media objects, raw types AND miniproto file IDs so concurrency never silently degrades to sequential. Bench defaults (`DEFAULT_DOWNLOAD_CONCURRENCY=6`, `DEFAULT_DOWNLOAD_MEDIA_LANES=2`) and `.env.example` updated to match. Live A/B matrix still pending (run on the VPS).
...
- Done 2026-07-07: `_legal_request_limit()` picks the largest legal limit (power-of-two divisor of 1 MiB when non-precise; 1 KiB multiple when precise) that fits `min(part_size, window, boundary_room)` and never straddles a 1 MiB boundary; sub-minimum tails over-request one minimal chunk and truncate (the server clamps at EOF). Non-4KiB-aligned offsets and sub-4KiB part sizes auto-enable `precise` per transfer (metric `media.download.precise_auto_enabled`); sub-1KiB offsets raise; `resume` realigns partial tails down to the last 1 KiB boundary and re-downloads them (`media.download.resume_realigned`). `part_size`/`max_part_size` must be powers of two (validated). Seeded property test (`test_download_requests_always_satisfy_telegram_alignment_rules`) asserts every emitted request against the documented rules.
...
- Done 2026-07-07 (REVISED same day after a live failure; tuned after live P1 review; revised again 2026-07-08 after a user-mode 1000 MiB run still showed `FloodPremiumWait` stalls): the first implementation released a flood-sleeping task's slot+bytes back to the window and backfilled it with new requests. The first live c6/l2 run falsified that design: six requests flooded together, their freed slots were instantly backfilled, the request rate never dropped, and the server escalated FLOOD_WAIT_2 → FLOOD_WAIT_15 (8161 part retries by the 37 MiB mark) until one part burned its whole retry budget on consecutive floods and aborted the transfer. Final behavior: **flood-sleeping tasks HOLD their slot** (mtcute's fixed-slot model — pressure drops naturally, other slots keep flowing); **floods never consume the transient retry budget** (separate cap of 16 per part, uploads too — the download abort path is gone); `FLOOD_WAIT_0` gets a 1 s floor, positive waits use the server's wait, and all flood sleeps add only 0–0.3 s jitter so lockstep-flooded requests de-synchronize without adding hundreds of seconds of avoidable stall. After generic floods, a launch pacer re-arms the 50 ms stagger and spaces new launches under the recent successful request rate without dropping below 4 launches/s; after `FloodPremiumWait`, the adaptive throttle switches the current download to one active request for the rest of that transfer because the 2026-07-08 c1/l1 fixed-1MiB comparison was the only user run without premium-flood collapse. Benchmark JSON reports launch-pacing wait/rate counters so future runs can prove whether pacing engaged. The concurrency limit still never reduces on generic `FloodWait` (growth pause only), while disconnect/timeout errors shrink by one with a 2 s cooldown. Delegated-flood `rpc.invoke` logs are INFO (the 3-lines-per-flood ERROR wall is gone).
...
- Done 2026-07-07: pools are keyed `(kind, dc_id)` with a grow-only `ensure_lanes()` resize (c1 then c2 reuses the warm socket instead of building a disjoint pool); `_MediaInvokeContext` prewarms all lanes in parallel (best-effort, `client.media_lane_prewarm_errors`) before the first request; a per-pool idle reaper closes lanes with no requests for `ClientConfig.media_idle_close` seconds (default 120 s, `None` disables — user decision 2a) and the next transfer transparently rebuilds. `media_lanes=0` escape hatch unchanged.
...
- Done 2026-07-07 (fake-backed; live cross-DC run pending): media pools target the file's DC — `download_media` resolves `Media.dc_id`/file-id DC and builds the pool for it directly. Foreign-DC lanes get a dedicated per-DC auth key (key exchange against the media-flagged DC option), cached in memory AND persisted in session metadata (`dc_auth`, user decision 3a) so restarts skip the exchange; each fresh lane sender then runs `initConnection(invokeWithoutUpdates(auth.importAuthorization))` with an authorization exported on the main sender (metrics `client.media_auth_exports/imports`, `client.media_dc_auth_reused`). `FILE_MIGRATE_X` on a media lane no longer migrates the session: `_invoke_via_sender(migrate_session=False)` re-raises and `_MediaInvokeContext` re-resolves its pool to the indicated DC and retries; subsequent parts ride the migrated pool. Fake tests cover FILE_MIGRATE redirects and direct known-DC routing with the main session's dc_id untouched.
...
- Done 2026-07-07 (v1 scope per user decision 4a): `_send_pending` drains pending acks into a `msg_container` [MsgsAck, request] with correct inner/envelope msg_id+seq_no ordering, requeueing the acks if the send fails (`sender.acks_piggybacked`); only one RPC ever shares a container so the "never two upload.getFile per container" rule holds by construction. Standalone threshold/age flushes remain as backstops for receive-heavy idle periods. The fake server unwraps client-sent containers (recording acks on `acks_received`) so handler-based tests keep working. Full grammers-style multi-request send-queue batching stays out of scope.
...
- Done 2026-07-07 (implementation; 3-run live confirmation pending): `upload_file` now defaults to `concurrency=8` (client `send_file` too, lanes stay 2) and reads one window of parts ahead of the sends via a bounded producer queue (user decision 5a): sync reads run on a worker thread (`asyncio.to_thread`), async readers are awaited, BytesIO skips the hop; md5/order preserved; a failing read unblocks the consumer and re-raises. Media parts default to a 45 s request timeout (`DEFAULT_UPLOAD_PART_TIMEOUT`, the live-bench value that produced 15–16.25 MiB/s; explicit timeouts still win).
...
- Done 2026-07-07: `connect()` best-effort sets TCP_NODELAY, SO_KEEPALIVE and >=1 MiB SO_RCVBUF/SO_SNDBUF on the connected socket (unit-tested against a real localhost server). The per-packet `asyncio.timeout` in `recv()` is replaced by one idle-watchdog task per connection (user decision 7a): it fires only when a read is actually waiting and `read_timeout` elapses without activity, aborting the connection so the blocked read surfaces as `TransportTimeout` (steady slow streams tested to survive multiple deadlines). Small writes skip `drain()` while the write buffer is under 256 KiB, keeping backpressure for 512 KiB upload parts.
...
- Done 2026-07-07: the sequential path now routes disk writes through the same `_ConcurrentDestinationWriter` pipeline as the concurrent path in a new order-preserving `sequential=True` mode (appends at the handle's position, no seeks — user handles and non-seekable streams keep their semantics) so file I/O rides the worker thread instead of the event loop. Progress callbacks in both paths are coalesced by `_ProgressReporter` (>=250 ms apart or every 8 parts, first call immediate, final call always delivered).
...
- Done 2026-07-08: added Rust-backed `mtproto_encode_message`/`mtproto_decode_message` with Rust-side OS random padding via `getrandom`, deterministic explicit-padding support for tests, Python fallback parity functions, and `mtproto/codec.py` wiring through the public envelope wrapper. `benchmark_native_fallback_crypto.py` now includes `mtproto_encode_message_16k` and `mtproto_decode_message_16k`; release Linux VPS measurements show native envelope encode/decode is faster, so the public wrappers route through native while retaining fallback parity for unavailable/broken native builds.
...
- Done 2026-08-13: added stateful Rust and pure-Python codecs for abridged, intermediate, and padded-intermediate, with arbitrary fragment/coalescing input, multi-event drain, real padded transport padding, early declared-length rejection, 64 KiB socket reads, typed quick-ACK/error events, capability-only fallback, and release of receive buffers above the 1 MiB retained threshold. The Windows 11/Python 3.14.7/winloop warmed 4,096-frame/72-byte gate recorded raw distributions and passed at 9.61x abridged, 5.16x intermediate, and 5.79x padded-intermediate over the committed legacy `readexactly` path; all transport and offline full-suite regressions passed.
...
- Done 2026-08-13: added a reviewed 30-entry manifest pinned to normalized Layer 228, schema-derived API descriptors plus one machine-readable static service section, generated Rust dispatch/source and Python metadata, selected generated-class hooks, exact generic fallback/capability routing, and native/fallback/malformed/property coverage for five variants of every entry. The warmed representative 256 KiB mix recorded raw distributions and passed the 1.5x target at 1.68x; schema `--check`, rustfmt, Clippy with warnings denied, 26 Rust tests, and the complete offline Python suite passed.
