...
> - Download c4/lanes4/window4M: 5.21 MiB/s, reconnects 317 (was 838) but **sender_drops 0 / lane builds 4**. A re-run with a clamped ping interval got WORSE (800 reconnects, all `TransportClosed`, median 0.24 s apart, zero `TransportTimeout`), which identified the true root causes: (a) `ping_delay_disconnect` arms a server-side disconnect timer that only a NEW ping of the same type resets — the "skip pings while busy" policy let armed timers kill busy connections; (b) `_reconnect()` did not check whether another task had already replaced the failed transport, so the send path and receive loop ping-ponged reconnects, each closing the other's fresh transport. Fixed post-run: pings now flow on a fixed cadence regardless of traffic, and `_reconnect(failed_transport=...)` skips when the transport was already replaced (`sender.reconnect_skipped` metric). Re-run pending.
