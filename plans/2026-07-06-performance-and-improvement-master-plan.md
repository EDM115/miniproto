...
- Done 2026-07-07: the concurrent scheduler was rebuilt around a `_TransferWindow` (slots+bytes) shared with the part tasks: a flood-sleeping task releases its budget via a `_WindowLease` for the whole sleep and re-acquires before retrying (`media.download.flood_capacity_releases`), and the scheduler wakes on capacity release (refill event) to launch replacement requests mid-sleep. Per user decision the throttle NEVER reduces concurrency on `FloodWait` (it only pauses growth); disconnect/timeout errors still shrink by one with a 2 s cooldown, and slow-start is gone — burst protection is now a launch stagger (`_DownloadDelayGate`, 50 ms ×0.8 → 3 ms floor, mtcute's constants). Fake-invoker test proves all other parts complete during a single request's flood sleep.
