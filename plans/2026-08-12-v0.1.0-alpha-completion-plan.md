...
Date: 2026-08-12  
Status: In progress; Waves 0 through 4 implementation completed on 2026-08-13, with TASK-082's strict docs CI lane waiting for Wave 5.
...
| Wheel matrix                   | CPython 3.13 and 3.14 on Linux x86_64, Windows x86_64, macOS x86_64, and macOS arm64, plus sdist                                                                | Each artifact must be installed and imported in a clean environment on its build platform.                                                                                                                                                                             |
...
Acceptance: `python -m tools.schema.generate --check` is clean; generated metadata says Layer 228/TDLib canonical; all requested new pins are committed outputs; the full offline baseline passes before feature work continues.
...
Status: Implementation complete on 2026-08-13. V1-CI-082 remains in progress only for the strict documentation job that cannot run until Wave 5 provides the site and generators.  
...
Status: In progress on 2026-08-13. Python/Rust/benchmark/wheel/sdist/schema jobs are implemented and contract-tested; the strict docs job and `docs-site/dist/` artifact wait for Wave 5.  

Primary file: `.github/workflows/ci.yml`, plus schema/docs/wheel workflows if separation is clearer.  
Jobs:

1. Python quality/test matrix for 3.13 and 3.14 with Ruff format/lint, ty, deterministic schema checks, and pytest.
...
5. Maturin artifact matrix for Linux x86_64, Windows x86_64, macOS x86_64/arm64 on CPython 3.13/3.14 plus sdist. Install each wheel into a clean environment and run imports/native smoke on its build runner.
...
CLI design: `uv run python -m tools.release_check` with explicit modes such as `--quick`, `--offline` (default release gate), `--artifacts-dir`, and separately guarded `--live`; no secret-bearing arguments are persisted.  
...
6. Provide one cross-platform docs entry point through `uv run python -m tools.docs` which orchestrates generation/checking and invokes the pinned pnpm scripts without deleting arbitrary user-owned paths. Keep direct pnpm commands documented for frontend-only iteration.
...
- Verify optional dependency groups (`dev`, `docs`) and runtime dependencies are minimal, pinned according to project policy, and platform markers resolve on the wheel matrix.
...
CI matrix procedure: repeat clean install/import on every selected OS/Python wheel, upload artifacts and summaries, and fail on a pure-Python `py3-none-any` wheel unless a deliberately separate fallback artifact policy is later approved.  
...
2. `uv run ruff check .`
3. `uv run ty check`
4. `uv run python -m tools.schema.generate --check` plus pin/source-consistency checks.
5. `uv run python -m tools.docs --check`, frozen-lockfile frontend install, `astro check`, the static Astro/Starlight/Pagefind build, link/route validation, and browser search smoke tests.
