...
Date: 2026-08-12  
Status: In progress; Waves 0 through 5 implementation and the explicitly non-Wave intermediary release-readiness bridge are complete. Wave 4 was stabilized locally on 2026-08-14 after the supplied hosted CI failure, Wave 5 received fresh local docs/reference/site/container acceptance on 2026-08-18, and the subsequent bridge aligned `0.1.0` declarations, refreshed eligible direct dependencies/tooling, and fixed the queued-rebind and MTProto correlation findings without starting Wave 6. A 2026-08-19 release-automation addendum broadens the manual build into one attested 24-wheel/sdist/crate candidate and adds a separate run-ID-verified OIDC publisher without claiming a hosted candidate or publication. TASK-082 includes a real CPython 3.14t source/native gate, dispatch-only multi-architecture release-artifact automation, and a strict docs build/deploy workflow; 3.13t was removed to keep current PyO3. The task still needs corrected hosted CI/docs runs and a manual complete artifact workflow run before Wave 6 final metadata/artifact acceptance.
