...
Date: 2026-08-12  
Status: In progress; Waves 0 and 1 completed on 2026-08-13, with TASK-084 remaining open until later-wave quick-ACK, plain-hash, shared-scheduler, and streaming-cancellation acceptance cases land.  
...
- TASK-084 remains in progress by design. Later feature waves must still add production-path quick-ACK frames/tokens, optional plain-download hash corruption, shared cross-transfer scheduler fairness, and `iter_download()` cancellation journeys before the tracker can change to `yes`.
