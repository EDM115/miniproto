...
Date: 2026-08-12  
Status: Waves 0 through 7 are locally complete; external release execution remains pending. Wave 4 was stabilized locally on 2026-08-14 after the supplied hosted CI failure, Wave 5 received fresh local docs/reference/site/container acceptance on 2026-08-18, and the subsequent bridge aligned `0.1.0` declarations, refreshed eligible direct dependencies/tooling, and fixed the queued-rebind and MTProto correlation findings. Wave 6 completed local metadata, Windows CPython 3.14 wheel/sdist/crate, clean-install, and canonical offline acceptance. Hosted CI, documentation, and all 24 wheel lanes then passed at `c81533922ba63c89337f3959b881109c3b7e5dec`, closing TASK-082; those runs predate the current build-once sdist/crate/attestation/manifest expansion and therefore do not close TASK-092. Wave 7 reconciles the planning corpus and prepares the inspectable TASK-094 handoff. The final-SHA hosted candidate, protected OIDC publication, tag, registries, and public immutable GitHub release remain user-controlled and unexecuted.
...
| Brand identity                 | Eight materially distinct logo concepts followed by an explicit user selection/refinement checkpoint                                                                                                                                      | Each concept has a real editable SVG counterpart. Packet Loom (concept 03) is the maintainer-approved provisional identity and supplies light/dark/monochrome marks, wordmark lockups, favicon, and social-preview derivatives until the community poll selects a replacement or confirms it.                                                                                                                                                                                                          |
...
| Protected-session crypto       | Expose explicit Rust and cryptography AES-256-GCM/Scrypt functions; select Rust per available capability for the normal protected-session path                                                                                            | Warming/interleaved evidence shows Rust wins the actual small, Scrypt-dominated session workload while cryptography wins larger AES-GCM diagnostics. Missing native imports/symbols fall back to cryptography; real operation/authentication failures are never retried through another backend.                                                                                                                                                                                                     |
| Runtime Python dependencies    | `cryptography` plus platform-marked `uvloop`/`winloop` are base dependencies                                                                                                                                                              | Normal, development, free-threaded, and clean-wheel acceptance install the same declared runtime set. The `dev` extra contains development tools only; the former `crypto-fallback` and `event-loop` extras no longer exist.                                                                                                                                                                                                                                                                         |
...
| Wheel matrix                   | Dispatch-only normal CPython 3.13/3.14 and free-threaded CPython 3.14t wheels on Linux glibc/musl x86_64 and aarch64/ARMv8 plus native Windows/macOS x86_64 and aarch64; ordinary CI retains sdist and runtime gates but builds no wheels | Twenty-four wheel lanes install each wheel with its declared dependencies in the target environment, assert `cryptography` and the platform event-loop backend, exercise the bundled native layer and protected sessions, run concurrent native calls and every installed CLI `--help` from the selected interpreter, and prove the GIL is disabled for 3.14t without injecting `PYTHON_GIL` into normal builds. CPython 3.13t and ARMv7 are excluded to keep the current dependency/toolchain line. |
...
## 3. Planning Baseline And Gap Classification (2026-08-12)

### 3.1 Schema baseline

- At plan creation, `tools/schema/update.py` fetched only the core.telegram.org schema JSON, schema HTML/TL mirror, RPC errors, and layer page; Telegram Desktop and TDLib were not yet inputs.
- At plan creation, `tools/schema/schema.json`, `tools/schema/schema.tl`, metadata, generated Python facades/shards/stubs, registry, error tables, and `docs/raw-api.md` were based on core Layer 223. The shipped Alpha uses canonical TDLib structure and the validated Telegram Desktop Layer 228 marker.
- The planning-baseline parser accepted constructor-ID declarations and structured comments but lacked explicit handling for TDLib built-in/combinator declarations without `#id`, source-specific layer markers, and multi-source provenance.
- The planning-baseline generator treated core JSON as canonical and hardcoded core-oriented provenance wording in several outputs.
...
| --------------------------- | --------------------------------------- | -------------------------: | ----: | --------: | --------------------------------------------------------------------------------------------------- |
| Repository/core pin at plan creation | Layer 223                               |                      2,303 | 1,546 |       757 | Historical baseline                                                                                 |
...
### 3.3 Requested implementation status at the planning baseline

| Area                             | Extent at plan creation or later explicitly dated amendment                                                                                                         | Missing work assigned to this plan                                                                                                                                                                   |
| -------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| TASK-093 metadata                | The post-Wave-5 bridge aligns Python, Rust, lockfile, and documentation-site declarations to `0.1.0`/Alpha. PyPI has the reserved `0.0.1a0` placeholder.            | In Wave 6, complete the clean package/crate metadata review and verify README rendering plus sdist/wheel/crate contents without treating declaration alignment as artifact acceptance.                 |
...
Status: In progress on 2026-08-19. Python/Rust/free-threaded/all-nine-offline-benchmark/sdist/schema jobs, the strict docs/Pages lane, and a separate dispatch-only complete release-artifact workflow are implemented and contract-tested. A local CPython 3.14t Windows wheel passes with the GIL disabled. The artifact workflow retains the 24-lane wheel matrix and adds the authoritative Python sdist, one build-once Cargo package, per-distribution GitHub attestations, and attested deterministic checksum/provenance sidecars. The corrected hosted CI rerun and manual complete artifact run remain pending.
...
1. Python quality/test matrix for 3.13 and 3.14 with Ruff format/lint, ty, deterministic schema checks, and pytest, plus a 3.14t job that installs the base `cryptography` and `uvloop` dependencies, builds the extension, requires `Py_GIL_DISABLED`, forces the GIL off, and exercises protected sessions plus concurrent native calls. CPython 3.13t is deliberately unsupported because the current PyO3 line no longer supports it.
2. Stable Rust 1.97 fmt/Clippy/tests, including native unit/property tests.
3. Docs job with the current Node 26/pnpm 11 toolchain, a frozen Astro/Starlight/Pagefind lockfile, Griffe/`griffe2md`, a pinned docs-only nightly and `cargo-docs-md`, committed-reference coverage/drift checks, `astro check`, static build, link validation, and browser search/route smoke tests; upload the complete `docs-site/dist/` artifact.
4. Benchmark smoke job producing machine-readable artifacts.
5. A manual-dispatch-only release-artifact workflow with 12 Linux wheel lanes—glibc and musl across x86_64 and aarch64/ARMv8—and 12 native Windows/macOS x86_64/aarch64 lanes, each crossed with normal CPython 3.13/3.14 and free-threaded 3.14t. Linux aarch64 builds and tests use QEMU and pinned official PyPA container digests. Install each wheel normally with its declared dependencies in its actual target environment, assert `cryptography` and the platform event-loop backend, and run ABI/GIL, native concurrency, protected-session, and all-script-help acceptance from the selected interpreter. Do not inject `PYTHON_GIL` into the manual matrix: normal interpreters reject disabling it during preinitialization, and 3.14t must demonstrate its default GIL-disabled state after importing miniproto. ARMv7 is excluded because its Rust musl target lacks published host tools and its Python dependency ecosystem is materially weaker. In separate jobs, build the authoritative Python sdist and exactly one Cargo source package in an isolated target directory; attest every distribution, reject incomplete ABI/platform coverage or metadata drift, and emit attested `SHA256SUMS` plus `release-manifest.json`. The ordinary CI workflow builds a validation sdist but no wheel.
...
2026-08-14 stabilization: the supplied hosted Python jobs revealed that pytest does not create the missing parent of the configured nested `--basetemp`, and that the affected uvloop 0.22.1 debug async-generator finalization path can segfault on Python 3.13 as well as 3.14. The workflow now creates `.tmp` first, while affected uvloop/winloop debug runs use the stdlib loop factory on both supported normal minors. CI's deterministic benchmark job was expanded from five commands/four release-check families to all eight then-current non-live families. Wheel production was then removed from ordinary CI and isolated in the dispatch-only full platform matrix. Dependency freshness was explicitly prioritized: PyO3 is current at 0.29.2, so unsupported CPython 3.13t was removed while 3.14t remains mandatory. A later same-day dependency decision restored `cryptography` and the platform event-loop backend to the base install; bundled Rust AES-GCM/Scrypt remains the selected protected-session path when its individual capability is available. Source/workflow contracts, native Rust tests, and the earlier dependency-free local `cp314t` Windows wheel run with `-X gil=0` passed, but that wheel predates the restored dependency acceptance; fresh hosted CI and wheel-matrix evidence remain pending.

Fresh follow-up verification before the dependency restoration: actionlint 1.7.12, Ruff format/lint, ty, deterministic schema freshness, the complete offline Python suite, Cargo formatting, strict all-target/all-feature Clippy, and all 27 Rust tests passed on current PyO3 0.29.2, `aes-gcm` 0.11.0, and `scrypt` 0.12.0. The canonical offline release check passed in 233.4 seconds, including all eight then-current non-live benchmark families, fresh artifact inspection, and clean import; the Wave 5 docs stage remained correctly pending. The rebuilt Windows `cp314t` wheel passed with the GIL disabled, no optional CFFI dependency, 512 concurrent native calls, protected-session round trips, and all sixteen then-installed CLI help paths. These artifact results remain historical evidence but are not final proof of the restored dependency-resolving workflow; hosted CI and the corrected manual 24-lane matrix remain separate outstanding gates.

Fresh verification after restoring the base dependencies and adding the session-crypto benchmark: Ruff format/lint, ty, deterministic schema freshness, actionlint 1.7.12, Cargo formatting, strict all-target/all-feature Clippy, all 27 Rust tests, and the canonical offline release check passed. The release check completed in 249.381 seconds with exit code 0; pytest collected 1,021 tests with 1,013 passed and eight intentional live/stress skips, all nine non-live benchmark commands passed, and only the not-yet-implemented Wave 5 docs stage remained pending. The clean Windows CPython 3.14 wheel install resolved `cryptography 50.0.0`, `winloop 0.6.3`, `cffi 2.1.1`, and `pycparser 3.0`; wheel metadata contains the three restored base requirements and all 17 installed scripts passed `--help` outside the checkout. The wheel SHA-256 is `90a2bcf0164acd6be6f3ebd3701b91f9b286e60389d4c014e5a1e3703955b4de`, and the sdist SHA-256 is `525a804222776f9535d4147b8fbdd6ccc571fc60ffc8ba17634ed0cd513436e6`. Five-run Windows measurements selected native Rust for the real protected-session workload (28.7388 ms median versus 38.9553 ms for cryptography) and Scrypt (28.4210 ms versus 42.6538 ms), while retaining the bulk AES-GCM crossover as diagnostic evidence rather than adding an unvalidated cross-platform size router. Hosted CI and the corrected dispatch-only 24-lane wheel run remain separate outstanding gates.
...
Dependency/session-crypto follow-up verification on 2026-08-14: `uv run miniproto-release-check --offline --artifacts-dir .tmp/session-crypto-release-20260814-final` returned exit code 0 in 249.381 seconds. All implemented stages passed, including the 1,021-test Python collection (1,013 passed, eight intentional live/stress skips), all 27 Rust tests, all nine non-live benchmark commands, the wheel/sdist build and inspection, and a clean dependency-resolving import; docs remained correctly pending for Wave 5. The clean environment installed `cryptography 50.0.0` and `winloop 0.6.3`, the wheel metadata declares the restored base requirements, and all 17 installed scripts passed `--help`. The wheel SHA-256 is `90a2bcf0164acd6be6f3ebd3701b91f9b286e60389d4c014e5a1e3703955b4de`, and the sdist SHA-256 is `525a804222776f9535d4147b8fbdd6ccc571fc60ffc8ba17634ed0cd513436e6`. No credentialed Telegram run was performed, and hosted plus corrected 24-lane wheel evidence remains pending.

## 10. Wave 5 — TASK-088 Through TASK-091 And The Documentation Website

Status: implementation complete on 2026-08-18. The committed site and reference generators are in place, Packet Loom concept 03 is the provisional production identity while the community poll remains open, and the remaining release work starts in Wave 6. Hosted Pages deployment is implemented and locally validated but is not claimed as executed until GitHub runs the trusted-master workflow.
...
Primary files: new `docs-site/package.json`, `docs-site/pnpm-lock.yaml`, `docs-site/astro.config.ts`, `docs-site/tsconfig.json`, `docs-site/src/content.config.ts`, `docs-site/src/components/**`, `docs-site/src/styles/**`, `docs-site/src/assets/**`, `.gitignore`, `pyproject.toml`, `uv.lock`, tests/workflows.  
Design:

1. Track the current Node.js 26 and pnpm 11 releases: CI installs pnpm through `pnpm/action-setup@v6`, requests Node 26 from `actions/setup-node@v6` with `check-latest: true`, and never invokes Corepack. Keep the reviewed package-manager/runtime values and Astro/Starlight/Pagefind-facing dependency ranges in `package.json`, commit `pnpm-lock.yaml`, and install with the frozen lockfile. No JavaScript dependency becomes a Python runtime dependency.
...
6. Provide one cross-platform docs entry point through a `miniproto-docs` `[project.scripts]` entry with a side-effect-free `--help` path. `uv run miniproto-docs` orchestrates generation/checking and invokes pnpm directly without deleting arbitrary user-owned paths. Keep direct pnpm commands documented for frontend-only iteration.
...
### V1-DOCS-007 — Create eight real-vector logo directions and provisionally integrate the poll leader
...
1. Before final theme work, write a constrained creative brief around miniproto's identity: compact MTProto SDK, speed, correctness, security, transport/data flow, Python plus Rust, Alpha but serious engineering. A paper-plane direction must have a distinct miniproto mechanism/twist rather than reproduce Telegram's mark; avoid generic crypto shields, illegible micro-details, and logo forms that only work on one background.
2. Generate four materially different initial concept families rather than color variants, then four additional families around other library/protocol ideas. Present all eight with an icon mark, `miniproto` wordmark lockup, light/dark preview, rationale, strengths, weaknesses, and likely site mood.
3. Deliver a true editable standalone `logo.svg` for every concept alongside its board and raster preview. Standalone marks use vector paths/shapes/gradients only, include a `viewBox` and accessible title, and contain no `<image>`, raster payload, external asset, or font dependency. Presentation boards may use a locally retained embedded WOFF2 subset for literal wordmarks, but have no network or installed-font dependency and consume the adjacent standalone mark directly.
4. Pause at the intentional visual decision gate after all eight concepts are available. The user selects a direction and guides refinements; Packet Loom is provisional and must not be silently converted into the permanent community choice.
...
Acceptance: eight inspectable concept families and eight true SVGs exist before selection; the provisional family has a complete accessible scalable asset set and is the sole identity used by the current site theme until the maintainer reports the poll result.

### V1-DOCS-008 — Build the product homepage, bespoke theme, navigation, and Pagefind experience

Primary files: `docs/index.md`, `docs-site/src/components/{Header,Hero,Search,Footer,MarkdownContent}.astro` as justified by the final design, `docs-site/src/styles/**`, `docs-site/astro.config.ts`, search metadata integration, browser tests.  
...
Primary files: `.github/workflows/docs.yml`, `docs-site/Dockerfile`, `docs-site/nginx.conf`, docs development/deployment page, optional `CNAME` only if the user supplies a domain later.  
Steps:

1. On approved branch/tag events, install frozen Python/Rust/Node docs dependencies, verify committed generation is current, run `astro check`, build Astro with Pagefind, validate links/search/routes, and upload `docs-site/dist/` as a normal downloadable artifact. On a trusted `master` push, download those same bytes and replace the root of `gh-pages` through `peaceiris/actions-gh-pages@v4` with `force_orphan: true`; never commit `docs-site/dist/` on the source branch.
...
3. Document and implement the exact VPS path: package the prepared root-base artifact in an unprivileged NGINX-on-Alpine runtime, and provide an optional Node.js-on-Alpine source-build target for builders with enough memory. The runtime contains no source tree, package manager, Python, Rust, or server-side search process.
4. Add host-agnostic base/site URL configuration so copied artifacts do not contain GitHub-only hardcoded paths. `/` is the default for local/VPS/static-host use; CI explicitly builds `/miniproto` for the GitHub project site without changing content files.

Acceptance: local/CI output is byte/functionally equivalent apart from configured canonical/base URL metadata; the deployment workflow replaces `gh-pages` with one root orphan commit containing only the latest validated site; the artifact works under the configured GitHub project path and independently at `/`. Local acceptance must additionally prove the Alpine image is healthy, unprivileged, serves nested and Pagefind routes, and preserves 404 behavior. Hosted branch contents remain unclaimed until the workflow executes.

### V1-DOCS-013 — Publish the source-grounded codebase knowledge map

Primary files: `docs/codebase/{STACK,STRUCTURE,ARCHITECTURE,CONVENTIONS,INTEGRATIONS,TESTING,CONCERNS}.md`, `docs/project/index.md`, `docs-site/astro.config.ts`, documentation contract tests.  
Adapt the seven deliverables from the `acquire-codebase-knowledge` workflow to repository conventions instead of copying its default root paths. Each document is handwritten, carries validated Starlight frontmatter with `generated: false`, lives under `/project/codebase/`, and ends with concrete source/config/test evidence. The public site includes all seven as a collapsed contributor-oriented group; generated per-symbol reference remains under `docs/reference/**` and is not duplicated. `CONCERNS.md` explicitly separates plan intent, implemented source, locally verified behavior, hosted/live evidence gaps, and deferred Wave 6/7 work. Raw scanner output and temporary churn data remain ignored discovery artifacts rather than published sources of truth.

Acceptance: exactly seven adapted documents exist; all template areas are answered without placeholder tokens; links/routes/frontmatter build successfully; no generated Telegram surface is mistaken for maintained handwritten code; unresolved questions are either answered from source or recorded with their concrete release impact.

### Wave 5 completion evidence

- Maintained Python and Rust documentation audits report no missing prose or explicit parameter descriptions; the raw generated Telegram surface is covered by the separate schema extractor.
- Deterministic reference generation commits 517 Python pages, 62 Rust pages, and 3,946 Telegram pages, with source hashes, routes, kinds, aliases, and provenance in `docs/reference/manifest.json`.
- The Astro Starlight build emits 4,584 static pages, the Pagefind index, sitemap, base-aware routes, the Packet Loom derivatives, and a downloadable/deployable static artifact. `/` is the default; the docs workflow supplies `/miniproto`, then publishes the exact tested artifact to the root of an orphan `gh-pages` branch. Browser acceptance covers product positioning, nested navigation/sidebar completeness, cross-language/parameter/prose queries, facets, keyboard focus, narrow viewports, and both themes.
- Release-facing README, changelog, security policy, handwritten information architecture, portable deployment guide, and the seven source-grounded codebase documents are complete. The changelog describes the complete `0.1.0` Alpha product rather than private pre-release chronology.
- The default Docker target packages the prepared root-base output in unprivileged NGINX 1.31.3 on Alpine 3.24. Fresh acceptance rebuilt a 75,395,136-byte image, reached healthy state as UID 101, returned 200 for `/`, `/start/installation/`, and `/pagefind/pagefind.js`, returned 404 for a missing route, and verified revalidation headers for stable Pagefind/brand assets while retaining immutable caching only for content-hashed Astro assets. The optional Node.js 26.7.0/pnpm 11.22.0 Alpine `source-runtime` target is implemented and documented for builders with at least 2 GiB; its full build is not claimed on the local 938 MiB Docker daemon.
- Fresh local gates on 2026-08-18: schema and committed-reference freshness, Ruff formatting/lint, ty, actionlint 1.7.12, 1,093 Python tests passed with four opt-in stress tests skipped, all 27 Rust tests, 62 focused documentation tests, Packet Loom derivative checks, Astro diagnostics, the Node.js 26.7.0/pnpm 11.22.0 production build, 600,658-reference static route/link/search validation, and all six browser tests under both `/` and the exact `/miniproto` Pages base passed. Credentialed Telegram, hosted CI/Pages, and the dispatch-only 24-lane wheel workflow remain separate unclaimed gates.

### Intermediary release-readiness bridge after Wave 5 (not Wave 6)

Status: locally complete on 2026-08-18. Python, Rust, lockfile, and documentation-site declarations agree on `0.1.0` Alpha. Eligible direct Python documentation/quality dependencies were refreshed after upstream-changelog review; current Rust and remaining Python/pnpm dependencies were retained; Astro-incompatible major/package mismatches were not forced. The docs site now uses Oxfmt for supported sources and type-aware Oxlint for maintained `.ts`; every maintained JavaScript helper was converted to properly typed TypeScript executed through Jiti; all twenty Python entry points and both documentation helper CLIs provide side-effect-free `--help`; and the release checker flushes per-stage progress before its final report. All pull requests are documented as draft-first, and the Pages workflow preserves `gh-pages` history. Packet Loom derivatives reconcile with the canonical concept-03 mark.

Correctness hardening completed at the end of the bridge: queued scheduler waiters carry their current scheduler/state ownership across repeated rebinds, completed/cancelled waiters are discarded before dispatch charges capacity, immediate container ancestry is retained through prevalidation so every child must be older than its enclosing container, and pending ping requests retain the expected ping ID so mismatched Pongs cannot complete them. Focused red-to-green regressions, both complete affected suites, the full 1,105-test Python collection with 1,101 passed and four opt-in stress skips, Ruff/ty, Cargo formatting/check/strict Clippy and 27 tests, Oxide/Astro/peer/brand gates, committed-reference freshness, and the real nine-stage quick release command including the full root-base docs build/static/browser acceptance passed. At that bridge checkpoint Wave 6 had not started, so clean distribution contents, hosted matrices, and release handoff remained separate gates; Section 11 records their later state.

### Release-automation addendum after the bridge (not Wave 6 acceptance)

Status: locally implemented on 2026-08-19. `.github/workflows/build-wheels.yml` remains manual-only but is now named **Build release artifacts** and yields one coherent candidate: the existing 24 target-tested wheels, an authoritative Python sdist, one Cargo source package built in an isolated target directory, a GitHub artifact attestation for every distribution, and attested deterministic checksum/provenance sidecars. `tools/release_artifacts.py` rejects wrong counts, missing/duplicate ABI-platform coverage, unexpected files, mismatched embedded Python/Cargo metadata, altered bytes, wrong run/version/commit context, and malformed checksums; its twentieth installed Python launcher has side-effect-free help.

`.github/workflows/publish-release.yml` is a separately dispatched, globally permission-empty publisher. It verifies the supplied build run through the Actions API, checks the source commit is on `master`, downloads that exact run, verifies the manifest/checksums and every build-workflow attestation, and only then enters protected isolated jobs. Those jobs populate a draft GitHub release, publish Python files through PyPI OIDC, publish the Cargo package through crates.io OIDC, require the registry `.crate` to match the attested build bytes, and publish the GitHub release last. The operator guide records one-time Trusted Publisher/environment/immutable-release setup, exact local and remote commands, token-only emergency paths, and partial-failure recovery. The current crates.io package remains a provenance/version-parity `cdylib` named `miniproto_native`; direct Rust consumption is deferred to the separate 2026-08-19 plan.

Local evidence is intentionally narrower than Wave 6: `uv lock --check`, Ruff formatting/lint, ty, Actionlint 1.7.12, all release/workflow/CLI/documentation policy contracts, the complete 1,117-item Python collection (1,113 passed and four stress skips), Cargo formatting/check/strict Clippy, and all 27 Rust tests passed. The local environment enabled and passed the four credentialed integration tests; that does not replace hosted platform evidence. Normalized Cargo metadata confirms `miniproto 0.1.0`, crate-local `README.md`, crates.io-only publication, and the `miniproto_native` `cdylib`; `cargo package --offline --allow-dirty` listed, extracted, and built the intended 12-file source package in an isolated temporary target. `--allow-dirty` was necessary only because these authorized edits are uncommitted and does not constitute release evidence. The regenerated reference changed only `_cli.py` provenance; the strict site gate built 4,584 pages, validated 600,661 local references and seven Pagefind facets, and passed all seven browser tests. No clean committed candidate, hosted matrix, OIDC registry setting, tag, publication, or immutable release is claimed.

## 11. Wave 6 — TASK-093 Metadata, TASK-092 Artifacts, And Final Verification

Execution status on 2026-08-19: V1-REL-093 and V1-REL-087 are complete locally. The exact `uv run miniproto-release-check --offline --artifacts-dir .tmp/wave6-20260819-final2` invocation passed all 22 ordered stages, including schema/reference freshness, Ruff, ty, the complete offline Python suite, Rust formatting/strict Clippy/tests, all nine benchmark/profile families, fresh wheel/sdist construction and inspection, and independent dependency-resolving clean installs from both artifacts. The run reported 1,114 passed and eight explicitly excluded live/opt-in tests, 27 passed Rust tests, Python 3.14.7, Rust 1.97.1, uv 0.12.5, Ruff 0.16.3, and ty 0.0.72. `twine check` passed the resulting wheel and sdist. A Cargo-native `cargo package --locked -p miniproto` from the exact extracted sdist packaged and verified 11 crate files without `--allow-dirty`. V1-REL-092 remains in progress until the user-owned committed/pushed SHA is exercised by the complete hosted 24-wheel workflow; no credentialed Telegram run, tag, registry publication, or GitHub release is claimed.
...
- Verify base runtime dependencies (`cryptography` plus platform-marked `uvloop`/`winloop`) and optional dependency groups (`dev`, later `docs`) are minimal, pinned according to project policy, and resolve on the normal and free-threaded wheel matrix. Clean-wheel acceptance must install and import the declared runtime dependencies rather than bypassing them with `--no-deps`.
...
Manual artifact-matrix procedure: dispatch `.github/workflows/build-wheels.yml`, repeat clean dependency-resolving install/import and runtime acceptance for every normal CPython 3.13/3.14 and free-threaded CPython 3.14t wheel across Linux glibc/musl x86_64/aarch64 and native Windows/macOS x86_64/aarch64, upload artifacts and summaries, and fail on a pure-Python `py3-none-any` wheel unless a deliberately separate fallback artifact policy is later approved. Every lane must import `cryptography` and the platform event-loop backend; free-threaded lanes must additionally assert both the `t` ABI and that the GIL is actually disabled without relying on `PYTHON_GIL`; all CLI executables must be resolved from the selected interpreter's scripts directory.  
...
Execution status on 2026-08-19: locally complete. `PLAN.md`, `PROGRESS.md`, `plans/README.md`, both requested July reports, this plan, the codebase-knowledge documents, live-test ledger, integration instructions, and release operator guide were reconciled against the current source, tests, workflows, local artifacts, and hosted run evidence. The post-reconciliation `uv run miniproto-release-check --offline --artifacts-dir .tmp/wave7-20260819-final` run passed all 22 stages: 1,114 Python tests passed with eight live/opt-in skips, all 27 Rust tests passed, all nine benchmark/profile families passed, 4,584 documentation pages and 600,661 local references validated with seven browser tests, both fresh distributions passed strict inspection, and clean dependency-resolving installs from the wheel and sdist succeeded. Its report records the working tree as dirty, so it is local implementation evidence and not a publishable candidate. TASK-082 is `yes` on the successful hosted CI/docs/24-wheel runs at `c81533922ba63c89337f3959b881109c3b7e5dec`; TASK-092 remains `in progress` for the complete final-SHA candidate; TASK-094 is `ready / user action required`. No Git mutation, tag, registry publication, or public release is part of Wave 7.
...
2. Update `PLAN.md` to the shipped Alpha architecture/defaults/scope and remove obsolete present-tense provenance or quick-ACK-deferred claims while preserving explicitly dated historical baselines.
...
Primary files: `docs/project/release.md`, `.github/workflows/build-wheels.yml`, `.github/workflows/publish-release.yml`, `tools/release_artifacts.py`, final release summary, TASK-094 row.  
Checklist:
...
- Provide exact user-owned commands for final clean verification, dispatching and reviewing one attested build run, publishing that run through protected PyPI/crates.io OIDC jobs, finalizing the populated immutable GitHub release, emergency local-token recovery, and post-publish clean installs.
...
- Documentation branding: preserve all eight true-vector concepts and the canonical derivative pipeline. Packet Loom is the explicit provisional identity; replace it only if the maintainer reports a different community-poll result and requests regeneration.
...
| Eight logo concepts, true SVG assets, user-guided refinement, product homepage            | V1-DOCS-007/008                            |
