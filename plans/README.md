...
- [2026-08-12 v0.1.0 Alpha completion plan](2026-08-12-v0.1.0-alpha-completion-plan.md): active risk-first execution plan; Waves 0 through 4 implementation are complete, including multi-source Layer 228 schema generation, flood/session/fake-server hardening, shared media scheduling/streaming/hash verification, native transport framing, opt-in quick ACK, generated hot TL constructors, normalized benchmark/tglib/matrix tooling, all eight non-live CI benchmark families, sixteen installed tool CLIs, canonical release automation, genuine GIL-disabled CPython 3.14t gates, and a dispatch-only 30-lane wheel matrix covering glibc/musl x86_64/aarch64/armv7l plus native Windows/macOS x86_64/aarch64. CPython 3.13t is intentionally excluded so miniproto can stay on the current PyO3 line. The user-supplied hosted run exposed and led to corrections for pytest basetemp creation and affected uvloop debug runners on Python 3.13/3.14. Local offline stabilization and Windows 3.14t wheel evidence is green; TASK-082 remains in progress for a fresh hosted CI rerun, a manual wheel workflow run, and the strict docs CI lane that depends on Wave 5, which is next.
