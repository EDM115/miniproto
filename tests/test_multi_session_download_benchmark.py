...
def test_explicit_session_paths_come_from_cli() -> None:
    args = parse_args(
        ["--actor", "user", "--file-id", "mpf1_test", "--session", ".tmp/a.sqlite", "--session", ".tmp/b.sqlite"], {}
    )
...
def test_session_paths_can_come_from_env() -> None:
    ...
    assert resolve_session_paths(args, {}) == (Path(".tmp/a.sqlite"), Path(".tmp/b.sqlite"), Path(".tmp/c.sqlite"))


def test_default_session_paths_follow_actor_client_count_and_dc() -> None:
    args = parse_args(["--actor", "bot", "--file-id", "mpf1_test", "--clients", "3", "--dc-id", "4"], {})
...
def test_contiguous_plan_uses_fewer_workers_when_file_is_smaller_than_worker_count() -> None:
    ...
    assert [(item.index, item.offset, item.limit) for item in ranges] == [(0, 0, MIB), (1, MIB, MIB + 512 * 1024)]


def test_duplicate_auth_key_groups_ignore_missing_values() -> None:
    assert duplicate_auth_key_groups((None, "aaa", "bbb", "aaa", None, "bbb")) == {"aaa": (1, 3), "bbb": (2, 5)}
